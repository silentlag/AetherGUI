...
static void SetWindowsAutoStart(bool enable);
...
static bool IsDefaultPluginRepoOwner(const std::wstring& owner) {
	return owner.empty() || LowerWide(owner) == LowerWide(std::wstring(AETHERGUI_GITHUB_OWNER));
}

static std::wstring GetSourceCatalogOwner(const std::wstring& configuredOwner) {
	return configuredOwner.empty() ? std::wstring(AETHERGUI_GITHUB_OWNER) : configuredOwner;
}

static std::wstring GetSourceCatalogName(const std::wstring&, const std::wstring& configuredName) {
	return configuredName.empty() ? std::wstring(AETHERGUI_GITHUB_REPO) : configuredName;
}

static std::wstring GetSourceCatalogRef(const std::wstring&, const std::wstring&, const std::wstring& configuredRef) {
	return configuredRef.empty() ? L"main" : configuredRef;
}
...
static bool IsCatalogStopWord(const std::wstring& word) {
	return word == L"otd" || word == L"aether" || word == L"plugin" ||
		...
		word == L"interpolator" || word == L"abstract" || word == L"qbit";
}
...
static bool ReadAetherLuaMetadata(const std::wstring& path, std::wstring& name, std::wstring& description, std::vector<PluginOptionMetadata>& options) {
	std::string source;
	if (!AetherLuaReadFileBytes(path, source))
		return false;
	...
	AetherLuaSandbox(L);
	std::string error;
	if (!AetherLuaDoBuffer(L, source, error)) {
		lua_close(L);
		return false;
	}
	std::string metaName;
	std::string metaDescription;
	std::vector<AetherLuaOption> luaOptions;
	bool ok = AetherLuaReadMetadata(L, metaName, metaDescription, luaOptions);
	if (ok) {
		if (!metaName.empty())
			name = Utf8ToWide(metaName);
		if (!metaDescription.empty())
			description = Utf8ToWide(metaDescription);
		for (const AetherLuaOption& o : luaOptions) {
			PluginOptionMetadata m;
			m.type = o.type;
			m.key = o.key;
			m.label = o.label.empty() ? Utf8ToWide(o.key) : Utf8ToWide(o.label);
			m.minValue = (float)o.minValue;
			m.maxValue = (float)o.maxValue;
			m.defaultValue = (float)o.defaultValue;
			if (!o.format.empty())
				m.format = Utf8ToWide(o.format);
			options.push_back(m);
		}
	}
	lua_close(L);
...
bool AetherApp::Initialize(HWND hwnd) {
	...
	sidebar.AddTab(L"Diagnostics", L"\xE7C3");
...
void AetherApp::Shutdown() {
	if (bgReadThread.joinable()) bgReadThread.join();
	driver.Stop();
	renderer.Shutdown();
}
...
void AetherApp::InitControls() {
	...
	filters.smoothingInterval.Layout(cx + hw + 16, 0, hw, L"Interval (ms)", 1, 16, 2, L"Derived from Latency (half its value, clamped 1..16). Sent to driver as SmoothingInterval.");
	filters.antichatterEnabled.Layout(cx, 0, L"Antichatter", L"Suppresses opposite-direction micro-jitter while the pen rests or moves slowly. Lives under Smoothing.");
	filters.antichatterStrength.Layout(cx, 0, hw, L"Strength", 0.0f, 1.0f, 0.3f, L"How aggressively micro-jitter is suppressed. 0..1 in 0.1 steps. Pairs with Multiplier.");
	filters.antichatterMultiplier.Layout(cx + hw + 16, 0, hw, L"Multiplier", 0.0f, 5.0f, 1.0f, L"Overall scaling of the weight curve.");
	...
	filters.temporalFollow.Layout(cx + hw + 16, 0, hw, L"Follow (mm)", 0.0f, 5.0f, 0.0f, L"Cursor trailing radius. 0 = off. Adds a smoothstep follow behind the pen.");
	...
	bgImageOpacity.Layout(cx, 0, cw, L"Image Opacity (%)", 5.0f, 60.0f, 16.0f, L"Visibility of the background image over the interface");
	dpiScale.format = L"%.0f";

	const wchar_t* btnOpts[] = { L"Disable", L"Mouse 1", L"Mouse 2", L"Mouse 3", L"Mouse 4", L"Mouse 5", L"Mouse Wheel", L"Custom" };
	...
	overclockEnabled.Layout(cx, 0, L"Interpolation", L"Interpolates extra output points between native tablet reports via a boosted timer. This is not a true USB polling overclock.");
	...
	penRateLimitHz.Layout(cx, 0, hw, L"Limit Rate (Hz)", 30, 1000, 133, L"Output report cap \u2014 only LOWERS the rate (drops extra reports). To RAISE a slow tablet's rate, use Interpolation instead. Example: 133 Hz gives Gaomon a Wacom-like cadence.");
	...
	rateMode.AddOption(L"Interpolation", L"Raise the output rate: a high-res timer interpolates extra points between native reports. Best for slow tablets.");
	...
	aether.enabled.Layout(cx, 0, L"Aether Glide", L"Adaptive multi-stage filter pipeline");
	aether.stabilizerEnabled.Layout(cx, 0, L"Stabilizer", L"Speed-adaptive smoothing: heavy at low speed, light at high speed.");
...
void AetherApp::CaptureSettingsSnapshot(SettingsSnapshot& snapshot) const {
	...
	slider(aether.stabilizerStability); slider(aether.stabilizerSensitivity); slider(aether.snappingInner); slider(aether.snappingOuter);
	...
	toggle(filters.smoothingEnabled); toggle(filters.antichatterEnabled); toggle(filters.reconstructorEnabled); toggle(filters.jitterStabEnabled); toggle(filters.temporalEnabled);
...
void AetherApp::ApplySettingsSnapshot(const SettingsSnapshot& snapshot) {
	...
	setSlider(aether.stabilizerStability); setSlider(aether.stabilizerSensitivity); setSlider(aether.snappingInner); setSlider(aether.snappingOuter); setSlider(filters.temporalPrediction); setSlider(filters.temporalSmoothing); setSlider(filters.temporalReverseEma); setSlider(filters.temporalFollow);
...
void AetherApp::ApplyDpiScale() {
	...
	// the slider already defaults to the system scale, so its value is the FINAL
	// scale - multiplying by the system DPI here applied it twice (1.25*1.25=1.5625)
	float uiScale = GetSelectedDpiScale();
...
void AetherApp::OnMouseMove(float x, float y) {
	...
	if (consoleSelecting && sidebar.activeIndex == 3) {
		int idx = ConsoleHitTest(x, y);
		if (idx >= 0) consoleSelEnd = idx;
	}
...
void AetherApp::OnMouseDown() {
	...
	mouseClicked = true;
	lastActivityTime = std::chrono::steady_clock::now();
	if (sidebar.activeIndex == 3 && consoleLayoutCount > 0 &&
		mouseX >= consoleRectX && mouseX <= consoleRectX + consoleRectW &&
		mouseY >= consoleRectY && mouseY <= consoleRectY + consoleRectH) {
		int idx = ConsoleHitTest(mouseX, mouseY);
		if (idx >= 0) {
			consoleSelStart = idx;
			consoleSelEnd = idx;
			consoleSelecting = true;
		}
	}
...
static bool IsWindowsAutoStartEnabled() {
    ...
    HKEY hKey;
    if (RegOpenKeyW(HKEY_CURRENT_USER, keyName, &hKey) != ERROR_SUCCESS) return false;
    wchar_t buf[1];
    DWORD type, size = sizeof(buf);
    bool exists = (RegQueryValueExW(hKey, L"AetherGUI", NULL, &type, NULL, &size) == ERROR_SUCCESS);
    RegCloseKey(hKey);
    return exists;
}
...
static void SetWindowsAutoStart(bool enable) {
    ...
    HKEY hKey;
    if (RegCreateKeyExW(HKEY_CURRENT_USER, keyName, 0, NULL, 0, KEY_SET_VALUE, NULL, &hKey, NULL) != ERROR_SUCCESS) return;
    if (enable) {
        wchar_t cmd[MAX_PATH + 64];
        swprintf_s(cmd, MAX_PATH + 64, L"\"%s\" --minimized", exePath);
        DWORD len = (DWORD)((wcslen(cmd) + 1) * sizeof(wchar_t));
        RegSetValueExW(hKey, L"AetherGUI", 0, REG_SZ, (const BYTE*)cmd, len);
    } else {
        RegDeleteValueW(hKey, L"AetherGUI");
    }
    RegCloseKey(hKey);
}

bool AetherApp::isAutoStartEnabled() const {
	return autoStartEnabled;
}

void AetherApp::toggleAutoStart() {
	autoStartEnabled = !autoStartEnabled;
	SetWindowsAutoStart(autoStartEnabled);
}
...
void AetherApp::ClampTabletAreaToFull(float fullTabletW, float fullTabletH) {
	if (area.tabletX.value <= 0.01f) area.tabletX.value = fullTabletW * 0.5f;
	if (area.tabletY.value <= 0.01f) area.tabletY.value = fullTabletH * 0.5f;
...
void AetherApp::OnResize(UINT width, UINT height) {
	...
	float s = Theme::Runtime::UiScale;
	if (s < 0.001f) s = 1.0f;
	Theme::Runtime::SetWindowSize(clientWidth / s, clientHeight / s);
...
void AetherApp::OnMouseWheel(float delta) {
	...
	case 3: consoleScrollY += delta * scrollSpeed; break;
	case 4: doctorScrollY -= delta * scrollSpeed; break;
	case 5: brushScrollY -= delta * scrollSpeed; break;
	}
...
void AetherApp::OnKeyDown(int vk) {
	if ((GetKeyState(VK_CONTROL) & 0x8000) && vk == 'C' && sidebar.activeIndex == 3 && consoleSelStart >= 0) {
		CopyConsoleSelectionToClipboard();
		return;
	}
	...
	if (capturingButtonMap >= 0 && capturingButtonMap < 3) {
		CycleSelector* btns[3] = { &buttonTip, &buttonBottom, &buttonTop };
		CycleSelector& btn = *btns[capturingButtonMap];
		if (vk == VK_ESCAPE) {
			btn.selected = 0; 
		...
		if (vk >= 0x41 && vk <= 0x5A) { 
			wchar_t label[8]; swprintf_s(label, L"Key %c", (wchar_t)vk);
			int found = -1;
			for (int j = 0; j < btn.optionCount; ++j) {
				if (wcscmp(btn.options[j], label) == 0) { found = j; break; }
			}
			if (found < 0 && btn.optionCount < 16) { btn.options[btn.optionCount++] = label; found = btn.optionCount - 1; }
			if (found >= 0) btn.selected = found;
		}
		capturingButtonMap = -1;
		ApplyAllSettings();
		AutoSaveConfig();
	...
	if (capturingHotkeySlot >= 0 && capturingHotkeySlot < kConfigHotkeyCount) {
		...
		if (alt)   mods |= MOD_ALT;
		if (win)   mods |= MOD_WIN;
		configHotkeys[capturingHotkeySlot].mods = mods;
		configHotkeys[capturingHotkeySlot].vk   = (UINT)vk;
		capturingHotkeySlot = -1;
	...
	if (runtimeMissingModalOpen && vk == VK_ESCAPE) {
		runtimeMissingModalOpen = false;
		return;
	}
	if (vmultiMissingModalOpen && vk == VK_ESCAPE) {
		vmultiMissingModalOpen = false;
		return;
	}
	if (calibrationOpen && vk == VK_ESCAPE) {
...
void AetherApp::Tick() {
	...
	if (bgImageBitmap && bgWantMax > bgDecodeMax + 256) {
	...
	if (!bgImagePath.empty() && !bgImageLoaded) {
		...
		if (bgReadThread.joinable() && bgReadDone.load()) {
			bgReadThread.join();
			driver.DebugLog("BGIMG", "worker done ok=%d %ux%u", bgReadOk ? 1 : 0, bgPixelW, bgPixelH);
			...
			if (bgReadOk && bgPixelW && bgPixelH) {
				...
				if (bmp) {
					...
					bgImageFailed = true;
					swprintf_s(bgImageErrorText, L"Image failed to load (0x%08X)", (unsigned)imgHr);
					driver.DebugLog("BGIMG", "bitmap wrap failed hr=0x%08X wasCache=%d", (unsigned)imgHr, bgReadWasCache ? 1 : 0);
				}
			} else {
				bgImageFailed = true;
				swprintf_s(bgImageErrorText, L"Image failed to load (0x8007006E)");
				driver.DebugLog("BGIMG", "decode failed wasCache=%d", bgReadWasCache ? 1 : 0);
				// a corrupt cache should not kill the image: drop it and retry once from the source path
				if (bgReadWasCache && !bgRetriedPath) {
					bgRetriedPath = true;
					DeleteFileW(GetBgCachePath().c_str());
					bgImageLoaded = false; bgImageFailed = false;
				}
			}
			// remember the decode in RAM so a device reset can re-wrap it instantly
			if (bgReadOk && bgImageBitmap) {
				bgLastPixels = std::move(bgPixels);
				bgLastW = bgPixelW; bgLastH = bgPixelH;
			}
			bgPixels.clear();
			bgPixels.shrink_to_fit();
			bgPixelW = bgPixelH = 0;
		}
		else if (!bgReadThread.joinable()) {
			// local cache first - it never touches the network
			std::wstring cache = GetBgCachePath();
			if (GetFileAttributesW(cache.c_str()) != INVALID_FILE_ATTRIBUTES) {
				driver.DebugLog("BGIMG", "kicking cache read: %S", cache.c_str());
				KickBgRead(cache);
			}
			else {
				driver.DebugLog("BGIMG", "no cache, kicking path read: %S", bgImagePath.c_str());
				KickBgRead(bgImagePath);
			}
		}
	}
	accentAnimT += deltaTime;
	...
	if (pendingTabletInfoResync && driver.isConnected &&
		...
		bool changed = (fabsf(driver.tabletWidth  - lastSeenTabletWidth)  > 0.01f) ||
		               (fabsf(driver.tabletHeight - lastSeenTabletHeight) > 0.01f);
	...
	if (sidebar.activeIndex != oldTab && oldTab >= 0) {
		...
		capturingActionHotkey = -1;
		openButtonDropdown = -1;
		capturingButtonMap = -1;
	}
	// slightly underdamped spring: egui-style glide with a hint of overshoot
	{
		float dt = deltaTime;
		...
		for (int i = 0; i < 2; i++) {
			...
			tabSpringT += tabSpringV * dt * 0.5f;
		}
		if (tabSpringT > 1.0f && tabSpringV > -0.01f && tabSpringV < 0.01f) {
			tabSpringT = 1.0f;
			tabSpringV = 0.0f;
		}
	}
	float easeT = tabSpringT;
	tabFadeAlpha = Clamp(easeT, 0.0f, 1.0f);
	...
	case 4: DrawDoctorPanel(); break;
	case 5: DrawBrushPanel(); break;
	case 6: DrawAboutPanel(); break;
	}
...

	{
		...
		case 4: contentH = doctorContentH; scrollY = doctorScrollY; scrollPtr = &doctorScrollY; break;
	case 5: contentH = brushContentH; scrollY = brushScrollY; scrollPtr = &brushScrollY; break;
			break;
		}
...
std::wstring AetherApp::GetConfigPath() {
	return GetConfigDirectory() + L"_autosave.cfg";
}
...
void AetherApp::SendFilterSettings() {
	...
	if (filters.temporalEnabled.value) {
		sprintf_s(cmd, "Temporal %.4f %.4f %.4f %.4f",
			filters.temporalPrediction.value, filters.temporalSmoothing.value,
			filters.temporalReverseEma.value, filters.temporalFollow.value);
		driver.SendCommand(cmd);
	} else {
...
void AetherApp::AutoLoadConfig() {
	...
	driver.DebugLog("CFG", "AutoLoad: marker=[%S] len=%u", lastNamed.c_str(), (unsigned)lastNamed.size());
	if (!lastNamed.empty()) {
		DWORD nattrs = GetFileAttributesW(lastNamed.c_str());
		driver.DebugLog("CFG", "named attrs=0x%08X", (unsigned)nattrs);
		if (nattrs != INVALID_FILE_ATTRIBUTES && !(nattrs & FILE_ATTRIBUTE_DIRECTORY)) {
			activeConfigPath = lastNamed;
	...
	DWORD attrs = GetFileAttributesW(path.c_str());
	driver.DebugLog("CFG", "autosave attrs=0x%08X [%S]", (unsigned)attrs, path.c_str());
...
void AetherApp::SaveLastLoadedMarker() {
	...
	if (_wfopen_s(&f, marker.c_str(), L"wb") != 0 || !f)
		return;
	int cb = WideCharToMultiByte(CP_UTF8, 0, activeConfigPath.c_str(), -1, nullptr, 0, nullptr, nullptr);
	if (cb > 0) {
		std::string utf8(cb - 1, '\0');
		WideCharToMultiByte(CP_UTF8, 0, activeConfigPath.c_str(), -1, utf8.data(), cb, nullptr, nullptr);
		fwrite(utf8.data(), 1, utf8.size(), f);
	}
	fclose(f);
}

std::wstring AetherApp::ReadLastLoadedMarker() {
	std::wstring marker = GetLastLoadedMarkerPath();
	FILE* f = nullptr;
	...
	while (!buf.empty() && (buf.back() == '\n' || buf.back() == '\r' ||
		...
		buf.pop_back();
	if (buf.empty()) return {};
	int cw = MultiByteToWideChar(CP_UTF8, 0, buf.c_str(), -1, nullptr, 0);
	if (cw <= 0) return {};
	std::wstring out(cw - 1, L'\0');
	MultiByteToWideChar(CP_UTF8, 0, buf.c_str(), -1, out.data(), cw);
	return out;
}
...
static std::wstring FormatHotkeyLabel(UINT mods, UINT vk) {
	...
	case VK_F3:  out += L"F3";  return out;
	case VK_F4:  out += L"F4";  return out;
	case VK_F5:  out += L"F5";  return out;
	case VK_F6:  out += L"F6";  return out;
	case VK_F7:  out += L"F7";  return out;
	case VK_F8:  out += L"F8";  return out;
	case VK_F9:  out += L"F9";  return out;
	case VK_F10: out += L"F10"; return out;
	case VK_F11: out += L"F11"; return out;
	case VK_F12: out += L"F12"; return out;
	case VK_SPACE: out += L"Space"; return out;
	case VK_TAB:   out += L"Tab";   return out;
	case VK_BACK:  out += L"Back";  return out;
	case VK_INSERT: out += L"Ins";  return out;
	case VK_DELETE: out += L"Del";  return out;
	case VK_HOME:   out += L"Home"; return out;
	case VK_END:    out += L"End";  return out;
	case VK_PRIOR:  out += L"PgUp"; return out;
	case VK_NEXT:   out += L"PgDn"; return out;
	}
...
std::wstring AetherApp::HotkeyLabelForSlot(int oneBasedIndex) const {
	if (oneBasedIndex < 1 || oneBasedIndex > kConfigHotkeyCount) return L"";
	const HotkeyBinding& b = configHotkeys[oneBasedIndex - 1];
	return FormatHotkeyLabel(b.mods, b.vk);
}
...
void AetherApp::UnregisterConfigHotkeys() {
	if (!hWnd) return;
	for (int i = 1; i <= kConfigHotkeyCount; ++i)
		UnregisterHotKey(hWnd, 0xA000 + i);
}

void AetherApp::RegisterConfigHotkeys() {
	if (!hWnd) return;
	UnregisterConfigHotkeys();
	int ok = 0, fail = 0, skipped = 0;
	for (int i = 0; i < kConfigHotkeyCount; ++i) {
		const HotkeyBinding& b = configHotkeys[i];
		if (b.vk == 0) { ++skipped; continue; }
		if (RegisterHotKey(hWnd, 0xA001 + i, b.mods | MOD_NOREPEAT, b.vk))
			++ok;
		else
			++fail;
	}
...
static LRESULT CALLBACK AetherActionHotkeyHookProc(int code, WPARAM wParam, LPARAM lParam) {
	if (code == HC_ACTION && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN)) {
		...
		if (vk != VK_CONTROL && vk != VK_LCONTROL && vk != VK_RCONTROL &&
			...
			vk != VK_LWIN && vk != VK_RWIN) {
			if (vk >= 0 && vk < 256) {
				if (g_hookKeyState[vk]) return CallNextHookEx(NULL, code, wParam, lParam);
				g_hookKeyState[vk] = 1;
			}
			bool ctrl  = (GetKeyState(VK_CONTROL) & 0x8000) != 0;
			bool shift = (GetKeyState(VK_SHIFT)   & 0x8000) != 0;
			...
			for (int i = 0; i < AetherApp::kActionHotkeyCount; ++i) {
				const AetherApp::ActionHotkey& h = app.actionHotkeys[i];
				if (h.vk == (UINT)vk && h.mods == mods && h.actionIndex > 0) {
					if (now - g_slotLastFireTick[i] >= 200) {
						g_slotLastFireTick[i] = now;
						PostMessageW(app.hWnd, WM_APP + 7, (WPARAM)i, 0);
					}
					break;
				}
			}
	...
	else if (code == HC_ACTION && (wParam == WM_KEYUP || wParam == WM_SYSKEYUP)) {
		KBDLLHOOKSTRUCT* kb = (KBDLLHOOKSTRUCT*)lParam;
		int vk = (int)kb->vkCode;
		if (vk >= 0 && vk < 256) g_hookKeyState[vk] = 0;
	}
...
void AetherApp::FireActionHotkey(int slot) {
	if (slot < 0 || slot >= kActionHotkeyCount) return;
	const ActionHotkey& h = actionHotkeys[slot];
	if (h.actionIndex <= 0 || h.actionIndex >= kActionHotkeyNameCount) return;
	wchar_t dbg[160];
	swprintf_s(dbg, L"[Aether] FireActionHotkey slot=%d actionIndex=%d\n", slot, h.actionIndex);
	OutputDebugStringW(dbg);
	switch (h.actionIndex) {
		case 1: filters.lazyMouseEnabled.value       = !filters.lazyMouseEnabled.value;       break;
		case 2: filters.pressureCurveEnabled.value   = !filters.pressureCurveEnabled.value;   break;
		case 3: filters.reconstructorEnabled.value   = !filters.reconstructorEnabled.value;   break;
		case 4: filters.jitterStabEnabled.value      = !filters.jitterStabEnabled.value;      break;
		case 5: filters.noiseEnabled.value           = !filters.noiseEnabled.value;           break;
		case 6: aether.enabled.value                = !aether.enabled.value;                break;
				case 7: filters.smoothingEnabled.value       = !filters.smoothingEnabled.value;       break;
		case 8: filters.antichatterEnabled.value     = !filters.antichatterEnabled.value;     break;
		case 9: filters.temporalEnabled.value       = !filters.temporalEnabled.value;        break;
		case 10: filters.predictionEnabled.value      = !filters.predictionEnabled.value;      break;
		default: return;
	}
	ApplyAllSettings();
	AutoSaveConfig();
...
void AetherApp::SaveAppProfiles() {
	...
	char header[64];
	sprintf_s(header, "AppProfilesEnabled=%d\n", appProfilesEnabled ? 1 : 0);
	fwrite(header, 1, strlen(header), f);

	for (const AppProfileEntry& e : appProfiles) {
		int cb1 = WideCharToMultiByte(CP_UTF8, 0, e.processName.c_str(), -1, nullptr, 0, nullptr, nullptr);
		int cb2 = WideCharToMultiByte(CP_UTF8, 0, e.configPath.c_str(),  -1, nullptr, 0, nullptr, nullptr);
		if (cb1 <= 1 || cb2 <= 1) continue;
		std::string s1(cb1 - 1, '\0'), s2(cb2 - 1, '\0');
		WideCharToMultiByte(CP_UTF8, 0, e.processName.c_str(), -1, s1.data(), cb1, nullptr, nullptr);
		WideCharToMultiByte(CP_UTF8, 0, e.configPath.c_str(),  -1, s2.data(), cb2, nullptr, nullptr);
		std::string line = "AppProfile=" + s1 + "|" + s2 + "\n";
		fwrite(line.data(), 1, line.size(), f);
	}
	fclose(f);
}

void AetherApp::LoadAppProfiles() {
	appProfiles.clear();
	...
	while (pos < buf.size()) {
		size_t nl = buf.find('\n', pos);
		std::string line = buf.substr(pos, (nl == std::string::npos ? buf.size() : nl) - pos);
		...
		if (key == "AppProfilesEnabled") {
			appProfilesEnabled = (atoi(val.c_str()) != 0);
		}
		else if (key == "AppProfile") {
			size_t bar = val.find('|');
			if (bar == std::string::npos) continue;
			AppProfileEntry e;
			e.processName = Utf8ToWide(val.substr(0, bar));
			e.configPath  = Utf8ToWide(val.substr(bar + 1));
			if (!e.processName.empty() && !e.configPath.empty())
				appProfiles.push_back(e);
		}
	}
}

std::wstring AetherApp::GetForegroundProcessName() {
...
void AetherApp::PollForegroundProcess() {
	...
	if (_wcsicmp(name.c_str(), L"aether.exe") != 0 &&
		_wcsicmp(name.c_str(), L"aethergui.exe") != 0) {
		lastForeignForeground = name;
	}

	if (!appProfilesEnabled || appProfiles.empty()) return;
	if (name == lastForegroundProcess) return;

	{
		wchar_t buf[256];
		swprintf_s(buf, L"[Aether] foreground changed: %s (was %s); %d profile(s) configured\n",
			name.c_str(),
			lastForegroundProcess.empty() ? L"<none>" : lastForegroundProcess.c_str(),
			(int)appProfiles.size());
		OutputDebugStringW(buf);
	}
	...
	for (const AppProfileEntry& e : appProfiles) {
		{
			wchar_t buf[300];
			swprintf_s(buf, L"[Aether]   check: profile=%s vs %s -> %s\n",
				e.processName.c_str(), name.c_str(),
				(_wcsicmp(e.processName.c_str(), name.c_str()) == 0) ? L"MATCH" : L"no");
			OutputDebugStringW(buf);
		}
		if (_wcsicmp(e.processName.c_str(), name.c_str()) != 0) continue;
		DWORD attrs = GetFileAttributesW(e.configPath.c_str());
		if (attrs == INVALID_FILE_ATTRIBUTES || (attrs & FILE_ATTRIBUTE_DIRECTORY)) {
			wchar_t buf[300];
			swprintf_s(buf, L"[Aether]   path missing: %s\n", e.configPath.c_str());
			OutputDebugStringW(buf);
			continue;
		}

		activeConfigPath  = e.configPath;
		loadingFromFile   = true;
		LoadConfig(activeConfigPath);
		loadingFromFile   = false;
		suppressNamedAutosave = true;
		...
		RefreshConfigFiles();
		wchar_t buf[300];
		swprintf_s(buf, L"[Aether]   LOADED profile for %s -> %s\n",
			name.c_str(), e.configPath.c_str());
		OutputDebugStringW(buf);
		matched = true;
		break;
	}
	if (!matched) {
		wchar_t buf[160];
		swprintf_s(buf, L"[Aether]   no profile matched %s\n", name.c_str());
		OutputDebugStringW(buf);
	}
}
...
void AetherApp::AddAppProfileFromForeground() {
	...
	for (AppProfileEntry& e : appProfiles) {
		if (_wcsicmp(e.processName.c_str(), name.c_str()) == 0) {
			e.configPath = activeConfigPath;
			SaveAppProfiles();
			return;
		}
	}
...
void AetherApp::StartCalibration() {
	...
	for (int i = 0; i < 4; ++i) {
		calibrationCapturedX[i] = 0;
		calibrationCapturedY[i] = 0;
	}
}

void AetherApp::TickCalibration() {
	...
	if (!calibrationWaitingForLift) {
		if (active && pp > 0.05f) {
			...
			calibrationLastSeenValid = true;
			calibrationWaitingForLift = true;
		}
	}
	else {
...
void AetherApp::ApplyCalibrationResult() {
	...
	for (int i = 1; i < 4; ++i) {
		...
		maxY = std::max(maxY, calibrationCapturedY[i]);
	}
	float w = maxX - minX;
	float h = maxY - minY;
...
void AetherApp::DrawCalibrationModal() {
	if (!calibrationOpen) return;

	float W = clientWidth;
	float H = clientHeight;
	renderer.FillRect(0, 0, W, H, D2D1::ColorF(0, 0, 0, 0.55f));

	float modalW = 420.0f;
	float modalH = 260.0f;
	float mx = (W - modalW) * 0.5f;
	float my = (H - modalH) * 0.5f;
	renderer.FillRoundedRect(mx, my, modalW, modalH, 10, Theme::BgElevated());
	renderer.DrawRoundedRect(mx, my, modalW, modalH, 10, Theme::BorderAccent());

	renderer.DrawText(L"CORNER CALIBRATION", mx, my + 14, modalW, 24,
	...
	renderer.DrawText(step, mx, my + 42, modalW, 18,
	...
	if (calibrationWaitingForLift) {
		...
		swprintf_s(live, L"Holding... X=%.2f mm  Y=%.2f mm",
			calibrationLastSeenX, calibrationLastSeenY);
	...
	for (int i = 0; i < calibrationStep && i < 4; ++i) {
		...
		renderer.DrawText(row, mx + 30, my + 150 + i * 18.0f, modalW - 60, 18,
	...
	bool hover = PointInRect(mouseX, mouseY, bx, by, btnW, btnH);
	D2D1_COLOR_F bg = hover
		? Theme::Error()
		: D2D1::ColorF(Theme::Error().r, Theme::Error().g, Theme::Error().b, 0.18f);
	renderer.FillRoundedRect(bx, by, btnW, btnH, 6, bg);
	renderer.DrawRoundedRect(bx, by, btnW, btnH, 6,
		hover ? Theme::BorderAccent() : Theme::BorderSubtle());
	renderer.DrawText(L"Cancel", bx, by, btnW, btnH,
		hover ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
...
bool AetherApp::LoadConfigByHotkey(int oneBasedIndex) {
	if (oneBasedIndex < 1) return false;
	int idx = oneBasedIndex - 1;
	if (idx >= (int)configEntries.size()) return false;

	const std::wstring path = configEntries[idx].path;
	DWORD attrs = GetFileAttributesW(path.c_str());
	if (attrs == INVALID_FILE_ATTRIBUTES || (attrs & FILE_ATTRIBUTE_DIRECTORY))
		return false;
...
void AetherApp::RefreshPluginList() {
	...
	if (folderFind != INVALID_HANDLE_VALUE) {
		do {
			...
			HANDLE luaFind = FindFirstFileW((folder + L"*.lua").c_str(), &luaData);
			if (luaFind != INVALID_HANDLE_VALUE) {
				do {
					if (luaData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
						continue;

					PluginEntry luaEntry;
					luaEntry.key = std::wstring(folderData.cFileName) + L"\\" + luaData.cFileName;
					luaEntry.name = folderData.cFileName;
					luaEntry.dllName = luaData.cFileName;
					luaEntry.enabled.value = true;
					luaEntry.enabled.animT = 1.0f;
					for (const PluginEntry& previous : previousEntries) {
						if (_wcsicmp(previous.key.c_str(), luaEntry.key.c_str()) == 0) {
							luaEntry.enabled.value = previous.enabled.value;
							luaEntry.enabled.animT = previous.enabled.value ? 1.0f : 0.0f;
							luaEntry.options = previous.options;
							break;
						}
					}
					ConfigurePluginDefaults(luaEntry);
					pluginEntries.push_back(luaEntry);
...
bool AetherApp::RefreshPluginCatalog() {
	...
	std::wstring sourceOwner = GetSourceCatalogOwner(pluginRepoOwner);
	std::wstring sourceName = GetSourceCatalogName(pluginRepoOwner, pluginRepoName);
	std::wstring sourceRef = GetSourceCatalogRef(pluginRepoOwner, pluginRepoName, pluginRepoRef);
	...
	if (pluginCatalogEntries.empty())
		pluginCatalogStatus = L"No Aether filters found in Plugins/ or local DLL plugins";
	else
		pluginCatalogStatus = std::to_wstring(pluginCatalogEntries.size()) + L" Aether filter(s)";
...
bool AetherApp::InstallLuaPluginWithDialog() {
	PrepareModalDialog();
	wchar_t filePath[MAX_PATH] = {};
	OPENFILENAMEW ofn = {};
	ofn.lStructSize = sizeof(ofn);
	ofn.hwndOwner = hWnd;
	ofn.lpstrFilter = L"Aether Lua Filter (*.lua)\0*.lua\0All Files (*.*)\0*.*\0";
	ofn.lpstrFile = filePath;
	ofn.nMaxFile = MAX_PATH;
	...
	std::wstring pluginsRoot = GetPluginDirectory();
	wchar_t fileName[_MAX_FNAME] = {};
	wchar_t fileExt[_MAX_EXT] = {};
	_wsplitpath_s(filePath, nullptr, 0, nullptr, 0, fileName, _MAX_FNAME, fileExt, _MAX_EXT);
	std::wstring destDir = pluginsRoot + L"\\" + fileName;
	std::wstring destPath = destDir + L"\\" + fileName + fileExt;
	CreateDirectoryW(pluginsRoot.c_str(), nullptr);
...
bool AetherApp::DownloadGitHubSourcePort(const PluginCatalogEntry& entry, std::wstring& folderPath, std::wstring& status) {
	...
	std::wstring repoOwner = GetSourceCatalogOwner(pluginRepoOwner);
	std::wstring repoName = GetSourceCatalogName(pluginRepoOwner, pluginRepoName);
	std::wstring repoRef = GetSourceCatalogRef(pluginRepoOwner, pluginRepoName, pluginRepoRef);
...
bool AetherApp::InstallRepositoryPlugin(PluginCatalogEntry& entry) {
	...
	pluginCatalogStatus = L"This entry is not installable automatically. Use Install DLL or Build Source.";
	return false;
}
...
void AetherApp::ClampScrollOffsets() {
	...
	if (doctorScrollY < 0) doctorScrollY = 0;
	...
	maxScroll = doctorContentH - visibleH;
	if (maxScroll < 0) maxScroll = 0;
	if (doctorScrollY > maxScroll) doctorScrollY = maxScroll;
}

static void RgbToHsv(float r, float g, float b, float& h, float& s, float& v) {
	float mx = fmaxf(fmaxf(r, g), b), mn = fminf(fminf(r, g), b), d = mx - mn;
	v = mx; s = mx > 0.0f ? d / mx : 0.0f; h = 0.0f;
	if (d > 0.00001f) {
...
static void HsvToRgb(float h, float s, float v, float& r, float& g, float& b) {
	int i = (int)(h * 6.0f);
	float f = h * 6.0f - i, p = v * (1.0f - s), q = v * (1.0f - f * s), t = v * (1.0f - (1.0f - f) * s);
	i %= 6;
	if (i == 0) { r = v; g = t; b = p; }
...
void AetherApp::UpdateAccentAnimation() {
	...
	if (accentAnimMode == 0) {
		if (prevMode != 0) {
			Theme::Custom::SetAccent(uiAccentBaseR, uiAccentBaseG, uiAccentBaseB);
		...
		lastR = uiAccentBaseR; lastG = uiAccentBaseG; lastB = uiAccentBaseB;
		return;
	}
	if (fabsf(Theme::Custom::AccentR - lastR) > 0.001f ||
		fabsf(Theme::Custom::AccentG - lastG) > 0.001f ||
		fabsf(Theme::Custom::AccentB - lastB) > 0.001f) {
		uiAccentBaseR = Theme::Custom::AccentR;
		uiAccentBaseG = Theme::Custom::AccentG;
		uiAccentBaseB = Theme::Custom::AccentB;
	}
	if (accentAnimMode == 1) {
		float f = 1.0f + 0.18f * sinf(accentAnimT * 2.4f);
		Theme::Custom::SetAccent(Clamp(uiAccentBaseR * f, 0.0f, 1.0f), Clamp(uiAccentBaseG * f, 0.0f, 1.0f), Clamp(uiAccentBaseB * f, 0.0f, 1.0f));
	}
	else {
		float h, s, v, r, g, b;
	...
	lastR = Theme::Custom::AccentR; lastG = Theme::Custom::AccentG; lastB = Theme::Custom::AccentB;
...
bool AetherApp::ChooseBackgroundImage() {
	...
	bgImageFailed = false;
	bgRetriedPath = false;
	bgImageErrorText[0] = 0;
	// keep a local copy next to the config so startup never waits on the original path
	if (!CopyFileW(filePath, GetBgCachePath().c_str(), FALSE))
		DeleteFileW(GetBgCachePath().c_str());
	AutoSaveConfig();
	return true;
}
...
std::wstring AetherApp::GetBgCachePath() {
	return GetConfigDirectory() + L"bgcache.img";
}

// full image pipeline on the worker thread: file read + WIC decode + scale + convert to
// premultiplied BGRA. A 4K PNG decoded on the UI thread froze the window for seconds.
static bool DecodeImageToPixels(const std::wstring& path, std::vector<unsigned char>& outPx, UINT& outW, UINT& outH, UINT maxSize) {
	std::ifstream in(path, std::ios::binary);
	...
	if (in.gcount() != (std::streamsize)len) return false;
	...
	do {
		...
		if (maxSize > 0 && (w > maxSize || h > maxSize)) {
			...
			if (w > h) dh = (UINT)((float)maxSize * h / w);
			else if (h > w) dw = (UINT)((float)maxSize * w / h);
			if (FAILED(f->CreateBitmapScaler(&scaler))) break;
			if (FAILED(scaler->Initialize(frame, dw, dh, WICBitmapInterpolationModeHighQualityCubic))) break;
			src = scaler;
			w = dw; h = dh;
		}
		if (FAILED(f->CreateFormatConverter(&conv))) break;
		if (FAILED(conv->Initialize(src, GUID_WICPixelFormat32bppPBGRA, WICBitmapDitherTypeNone, nullptr, 0.0f, WICBitmapPaletteTypeMedianCut))) break;
		UINT cw = 0, ch = 0;
		conv->GetSize(&cw, &ch);
		UINT stride = cw * 4;
		if (cw == 0 || ch == 0 || stride / 4 != cw) break;
		outPx.resize((size_t)stride * ch);
		WICRect rc = { 0, 0, (INT)cw, (INT)ch };
		if (FAILED(conv->CopyPixels(&rc, stride, (UINT)outPx.size(), outPx.data()))) { outPx.clear(); break; }
		outW = cw; outH = ch;
		ok = true;
	} while (false);
...
void AetherApp::KickBgRead(const std::wstring& path) {
	...
	bgReadThread = std::thread([this, path]() {
		...
		if (DecodeImageToPixels(path, px, w, h, wantMax)) {
			bgPixels = std::move(px);
			bgPixelW = w; bgPixelH = h;
			bgReadOk = true;
		}
...
void AetherApp::DrawSegmentedRow(const wchar_t* const* names, int count, int* value, float x, float y, float w) {
	float bw = (w - (count - 1) * 4.0f) / (float)count;
	for (int i = 0; i < count; i++) {
		float bx = x + i * (bw + 4.0f);
		bool active = (i == *value);
		bool hov = PointInRect(mouseX, mouseY, bx, y, bw, 26.0f);
		if (active) {
			renderer.FillRoundedRect(bx, y, bw, 26.0f, 6, Theme::AccentPrimary());
			renderer.DrawText(names[i], bx, y, bw, 26.0f, D2D1::ColorF(0xFFFFFF), renderer.pFontSmall, Renderer::AlignCenter);
		} else {
			renderer.FillRoundedRect(bx, y, bw, 26.0f, 6, hov ? Theme::BgHover() : Theme::BgElevated());
			renderer.DrawRoundedRect(bx, y, bw, 26.0f, 6, Theme::BorderSubtle());
			renderer.DrawText(names[i], bx, y, bw, 26.0f, hov ? Theme::TextPrimary() : Theme::TextMuted(), renderer.pFontSmall, Renderer::AlignCenter);
		}
		if (hov && mouseClicked)
			*value = i;
	}
...
void AetherApp::DrawHeader() {
	...
	if (w > 640.0f && sidebar.activeIndex >= 0 && sidebar.activeIndex < (int)sidebar.tabs.size()) {
		renderer.DrawText(sidebar.tabs[sidebar.activeIndex].label, titleX + 196.0f, 0, 160, h,
			Theme::TextMuted(), renderer.pFontSmall);
	}

	
	float stopW = 72.0f;
	float stopX = w - stopW - 14.0f;
	startStopBtn.x = stopX;
	startStopBtn.y = 9;
	startStopBtn.width = stopW;
	startStopBtn.label = driver.isConnected ? L"Stop" : L"Retry";
	startStopBtn.isPrimary = !driver.isConnected;

	
	float startupW = 96.0f;
	float startupX = stopX - startupW - 10.0f;
	if (startupX > titleX + 220.0f && w > 660.0f) {
		autoStartBtn.x = startupX;
		autoStartBtn.y = 11;
		autoStartBtn.width = startupW;
		autoStartBtn.height = 26;
		autoStartBtn.isPrimary = false;
		autoStartBtn.label = autoStartEnabled ? L"Startup on" : L"Startup off";
		autoStartBtn.tooltip = L"Start AetherGUI when Windows starts";
		bool clicked = autoStartBtn.Update(mouseX, mouseY, mouseClicked, deltaTime);
		autoStartBtn.Draw(renderer);
		if (clicked) toggleAutoStart();
	}

	
	float pillW = 96.0f;
	float pillX = startupX - pillW - 10.0f;
	if (pillX > titleX + 220.0f) {
		bool tabletOk = driver.isConnected && driver.tabletConnected.load();
		D2D1_COLOR_F pillBg = tabletOk ? Theme::AccentDim() : Theme::BgElevated();
		D2D1_COLOR_F pillTxt = tabletOk ? Theme::Success() : Theme::TextSecondary();
		D2D1_COLOR_F pillBorder = tabletOk ? Theme::BorderAccent() : Theme::BorderSubtle();
		renderer.FillRoundedRect(pillX, 11, pillW, 26, 13, pillBg);
		renderer.DrawRoundedRect(pillX, 11, pillW, 26, 13, pillBorder, 1.0f);
		renderer.DrawText(tabletOk ? L"Connected" : (driver.isConnected ? L"No Tablet" : L"Starting"),
			pillX, 11, pillW, 26, pillTxt, renderer.pFontSmall, Renderer::AlignCenter);
	}

	
	float chipW = 148.0f;
	float chipX = pillX - chipW - 10.0f;
	if (w > 840.0f && !displayTargets.empty() && chipX > titleX + 220.0f) {
		...
		float contentX = chipX + (chipW - totalW) * 0.5f;
		...
		renderer.DrawText(L"\xE7F4", contentX, 11, iconW, 26,
			Theme::AccentPrimary(), renderer.pFontIcon, Renderer::AlignCenter);
		renderer.DrawText(label.c_str(), contentX + iconW + gap, 11, textW, 26,
			Theme::TextSecondary(), renderer.pFontSmall, Renderer::AlignLeft);
	}
...
void AetherApp::DrawOverclockInfo(float x, float y, float w) {
	...
	if (latN > 0) {
		swprintf_s(latLine,
			L"Internal latency  average %.2f ms   usually below %.2f ms   worst spike %.2f ms   over %d samples",
...
void AetherApp::DrawAreaPanel() {
...

	{
		...
		if (driver.isConnected && driver.penActive.load()) {
			...
			if (area.tabletWidth.value > 0.1f && area.tabletHeight.value > 0.1f) {
				...
				float effX = (area.tabletX.value > 0.01f) ? area.tabletX.value : fullW * 0.5f;
				float effY = (area.tabletY.value > 0.01f) ? area.tabletY.value : fullH * 0.5f;
				tx = (px - (effX - area.tabletWidth.value * 0.5f)) / area.tabletWidth.value;
				ty = (py - (effY - area.tabletHeight.value * 0.5f)) / area.tabletHeight.value;
				tx = Clamp(tx, 0.0f, 1.0f);
				ty = Clamp(ty, 0.0f, 1.0f);
			}
			float dotX = previewX + (area.screenX.value + tx * area.screenWidth.value) * scale;
			float dotY = previewY + (area.screenY.value + ty * area.screenHeight.value) * scale;
...

	{
		float calW = 180.0f;
		float calH = 24.0f;
		float calX = cx + cw - calW;
		float calY = y - 30.0f;
		bool hover = PointInRect(mouseX, mouseY, calX, calY, calW, calH);
		D2D1_COLOR_F bg = hover ? Theme::AccentPrimary() : Theme::AccentDim();
		renderer.FillRoundedRect(calX, calY, calW, calH, 5, bg);
		renderer.DrawRoundedRect(calX, calY, calW, calH, 5,
			hover ? Theme::BorderAccent() : Theme::BorderSubtle());
		renderer.DrawText(L"Calibrate corners", calX, calY, calW, calH,
			hover ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
			renderer.pFontSmall, Renderer::AlignCenter);
		if (hover && mouseClicked) StartCalibration();
	}
...
void AetherApp::DrawSettingsPanel() {
	...
	{
		CycleSelector* btns[3] = { &buttonTip, &buttonBottom, &buttonTop };
		const wchar_t* labels[3] = { L"Pen Tip", L"Bottom", L"Top" };
		float gap = 8.0f;
		float cellW = (cw - gap * 2.0f) / 3.0f;
		float rowH = 38.0f;
		for (int bi = 0; bi < 3; ++bi) {
			CycleSelector& btn = *btns[bi];
			float boxX = cx + bi * (cellW + gap);
			btn.x = boxX; btn.y = y; btn.width = cellW;
			bool capturing = (capturingButtonMap == bi);
			bool valHover = PointInRect(mouseX, mouseY, boxX, y, cellW, rowH);
			const wchar_t* val;
			D2D1_COLOR_F bg, border, nameCol, valCol;
			if (capturing) {
				bg = Theme::AccentPrimary(); border = Theme::BorderAccent();
				nameCol = D2D1::ColorF(1,1,1,0.85f); valCol = D2D1::ColorF(0xFFFFFF);
				val = L"press key...";
			} else if (valHover) {
				bg = Theme::BgHover(); border = Theme::BorderNormal(); nameCol = Theme::TextSecondary(); valCol = Theme::AccentPrimary();
				val = (btn.selected >= 0 && btn.selected < btn.optionCount) ? btn.options[btn.selected] : L"Disable";
			} else {
				bg = Theme::BgElevated(); border = Theme::BorderSubtle(); nameCol = Theme::TextMuted(); valCol = Theme::TextPrimary();
				val = (btn.selected >= 0 && btn.selected < btn.optionCount) ? btn.options[btn.selected] : L"Disable";
			}
			renderer.FillRoundedRect(boxX, y, cellW, rowH, 6, bg);
			renderer.DrawRoundedRect(boxX, y, cellW, rowH, 6, border);
			renderer.DrawText(labels[bi], boxX + 8, y + 4, cellW - 16, 12, nameCol, renderer.pFontSmall);
			renderer.DrawText(val, boxX + 8, y + 18, cellW - 16, 16, valCol, renderer.pFontSmall, Renderer::AlignLeft);
			if (mouseClicked && valHover) {
				if (capturing) capturingButtonMap = -1;
				else capturingButtonMap = bi;
			}
		}
	}
	y += 48;
	...
	{
		const wchar_t* animModes[] = { L"Full", L"Calm", L"Off" };
		int prevAnimMode = animSpeedMode;
		DrawSegmentedRow(animModes, 3, &animSpeedMode, cx, y, cw);
		if (animSpeedMode != prevAnimMode) {
	...
	sec.Layout(cx, y, cw, L"BACKGROUND"); y += sec.Draw(renderer);
	{
		bgImageOpacity.x = cx; bgImageOpacity.y = y; bgImageOpacity.width = cw;
		...
		bgClearBtn.Draw(renderer);
		y += 32;
		if (!bgImagePath.empty()) {
			if (bgImageFailed)
				renderer.DrawText(bgImageErrorText, cx, y, cw, 18, Theme::Error(), renderer.pFontSmall);
			else
				renderer.DrawText(bgImagePath.c_str(), cx, y, cw, 18, Theme::TextMuted(), renderer.pFontSmall);
		}
		y += 20;
	}
...
void AetherApp::SaveConfig(const std::wstring& path) {
	...
	for (int i = 0; i < uiThemeCount; i++) {
		if (!themeEdited[i]) continue;
		// 6 editable slots as RRGGBB; loaded back before the UITheme line applies
		f << "ThemeEdit" << i << "=";
		for (int s = 0; s < THEME_SLOT_COUNT; s++) {
			float r = 0, g = 0, b = 0;
			GetThemeSlotColor(uiThemes[i], s, r, g, b);
			if (s) f << " ";
			char hex[8];
			sprintf_s(hex, "%02X%02X%02X", (int)(r * 255 + 0.5f), (int)(g * 255 + 0.5f), (int)(b * 255 + 0.5f));
			f << hex;
		}
		f << "\n";
	}
	...
	if (selectedDisplayTarget >= 0 && selectedDisplayTarget < (int)displayTargets.size()) {
		...
		int cb = WideCharToMultiByte(CP_UTF8, 0, dt.label.c_str(), -1, nullptr, 0, nullptr, nullptr);
		std::string utf8;
		if (cb > 0) {
			utf8.assign((size_t)(cb - 1), '\0');
			WideCharToMultiByte(CP_UTF8, 0, dt.label.c_str(), -1, utf8.data(), cb, nullptr, nullptr);
		}
		f << "DisplayTargetLabel=" << utf8 << "\n";
		f << "DisplayTargetRect=" << dt.x << " " << dt.y << " "
		                          << dt.width << " " << dt.height << "\n";
	}
...
void AetherApp::LoadConfig(const std::wstring& path) {
	driver.DebugLog("CFG", "LoadConfig: [%S]", path.c_str());
	std::ifstream f(path);
	if (!f.is_open()) { driver.DebugLog("CFG", "LoadConfig: FAILED to open"); return; }
	...
	float loadedDisplayX = 0, loadedDisplayY = 0, loadedDisplayW = 0, loadedDisplayH = 0;
	...
	while (std::getline(f, line)) {
		...
		if (key.rfind("ThemeEdit", 0) == 0) {
			int idx = atoi(key.c_str() + 9);
			if (idx >= 0 && idx < uiThemeCount) {
				std::istringstream ts(rawValue);
				std::string tok;
				for (int s = 0; s < THEME_SLOT_COUNT && ts >> tok; s++) {
					unsigned ri = 0, gi = 0, bi = 0;
					if (tok.size() < 6) break;
					sscanf_s(tok.c_str(), "%02x%02x%02x", &ri, &gi, &bi);
					SetThemeSlotColor(uiThemes[idx], s, ri / 255.0f, gi / 255.0f, bi / 255.0f);
				}
		...
		if (key == "BgImage") {
			// string key: must run before the stof() gate below, or the line is silently dropped
			bgImagePath = Utf8ToWide(rawValue);
			bgImageLoaded = false;
			bgRetriedPath = false;
			driver.DebugLog("BGIMG", "config path: %S", bgImagePath.c_str());
			continue;
		}
		...
		if (key == "DisplayTargetLabel") {
			loadedDisplayLabel = Utf8ToWide(rawValue);
			continue;
		}
		if (key == "DisplayTargetRect") {
			float dx = 0, dy = 0, dw = 0, dh = 0;
			if (sscanf_s(rawValue.c_str(), "%f %f %f %f", &dx, &dy, &dw, &dh) == 4) {
				loadedDisplayX = dx; loadedDisplayY = dy;
				loadedDisplayW = dw; loadedDisplayH = dh;
				hasLoadedDisplayRect = true;
			}
			continue;
		}
		...
		if (key == "TabletWidth") area.tabletWidth.value = val;
			...
			int slot = atoi(key.c_str() + 10);
			if (slot >= 1 && slot <= kConfigHotkeyCount) {
				unsigned int m = 0, vk = 0;
	...
	if (!displayTargets.empty()) {
		...
		if (hasLoadedDisplayRect) {
			for (int i = 0; i < (int)displayTargets.size(); i++) {
				const DisplayTarget& dt = displayTargets[i];
				if (fabsf(dt.x - loadedDisplayX) < 0.5f &&
				    fabsf(dt.y - loadedDisplayY) < 0.5f &&
				    fabsf(dt.width  - loadedDisplayW) < 0.5f &&
				    fabsf(dt.height - loadedDisplayH) < 0.5f) {
					resolved = i;
					break;
				}
			}
		}
		if (resolved < 0 && !loadedDisplayLabel.empty()) {
			for (int i = 0; i < (int)displayTargets.size(); i++) {
				if (displayTargets[i].label == loadedDisplayLabel) {
					resolved = i;
					break;
				}
			}
		}
		if (resolved < 0 && loadedDisplayIndex >= 0 &&
		                    loadedDisplayIndex < (int)displayTargets.size()) {
			resolved = loadedDisplayIndex;
		}
		if (resolved >= 0)
			selectedDisplayTarget = resolved;
	}
...
void AetherApp::DrawPluginFilterControls(float cx, float& y, float cw, float hw, float filterRightX, bool filterSingleColumn, bool& filterChanged) {
	...
	installLuaPluginBtn.Layout(cx + (btnW + gap), y, btnW, 28, L"Install Lua", false);
	if (installLuaPluginBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		InstallLuaPluginWithDialog();
		UpdatePluginCatalogInstallState();
	}
	installLuaPluginBtn.Draw(renderer);

	installSourcePluginBtn.Layout(cx + (btnW + gap) * 2.0f, y, btnW, 28, L"Build Source", false);
	...
	reloadPluginBtn.Layout(cx + (btnW + gap) * 3.0f, y, btnW, 28, L"Reload", false);
	...
	listPluginBtn.Layout(cx + (btnW + gap) * 4.0f, y, btnW, 28, L"List", false);
...
void AetherApp::DrawPluginSourceModal() {
	...
	if (pluginManagerApplySourceBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		...
		pluginRepoName = pluginRepoNameInput.buffer[0] ? pluginRepoNameInput.buffer : AETHERGUI_GITHUB_REPO;
		pluginRepoRef = pluginRepoRefInput.buffer[0] ? pluginRepoRefInput.buffer : L"main";
		pluginSourceEditorOpen = false;
		RefreshPluginCatalog();
	}
...
bool AetherApp::CheckVCRedist() {
	...
	for (const wchar_t* d : dlls) {
		...
		if (h) { FreeLibrary(h); continue; }
		if (!missingRuntimeNames.empty()) missingRuntimeNames += L", ";
		missingRuntimeNames += d;
	}
	return missingRuntimeNames.empty();
}

void AetherApp::CheckVMultiModeSelected() {
	if (!vmultiCheckDone || vmultiInstalled) return;
	if (outputMode.selected != 2 && outputMode.selected != 3) return;
	vmultiMissingModeName = (outputMode.selected == 3) ? L"Raw Absolute" : L"Windows Ink";
	vmultiMissingModalOpen = true;
}

void AetherApp::DrawVMultiMissingModal() {
	float overlayW = Theme::Runtime::WindowWidth;
	float overlayH = Theme::Runtime::WindowHeight;
	renderer.FillRect(0, 0, overlayW, overlayH, D2D1::ColorF(0, 0, 0, 0.55f));

	float w = std::min(540.0f, overlayW - 36.0f);
	float h = 240.0f;
	float x = (overlayW - w) * 0.5f;
	float y = (overlayH - h) * 0.5f;

	renderer.FillRoundedRect(x, y, w, h, 12, Theme::BgSurface());
	renderer.DrawRoundedRect(x, y, w, h, 12, Theme::Warning(), 1.4f);

	D2D1_COLOR_F glow = Theme::Warning();
	glow.a = 0.18f;
	renderer.FillCircle(x + 54, y + 54, 28, glow);
	renderer.DrawText(L"\xE7BA", x + 42, y + 40, 24, 28, Theme::Warning(), renderer.pFontIcon, Renderer::AlignCenter);

	renderer.DrawText(L"VMulti driver required", x + 92, y + 24, w - 116, 28, Theme::TextPrimary(), renderer.pFontBody);

	wchar_t bodyBuf[320];
	swprintf_s(bodyBuf,
		L"%s mode requires the VMulti driver, which is not installed on this system. Without it the mode falls back to Absolute. Click Download to get the VMulti installer, then restart the driver after installing.",
		vmultiMissingModeName.c_str());
	renderer.DrawText(bodyBuf, x + 92, y + 56, w - 116, 90, Theme::TextMuted(), renderer.pFontSmall);

	float btnY = y + h - 50;
	vmultiMissingLaterBtn.Layout(x + w - 250, btnY, 100, 30, L"Ignore", false);
	if (vmultiMissingLaterBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		vmultiMissingModalOpen = false;
	}
	vmultiMissingLaterBtn.Draw(renderer);
...
void AetherApp::DrawRuntimeMissingModal() {
	float overlayW = Theme::Runtime::WindowWidth;
	float overlayH = Theme::Runtime::WindowHeight;
	renderer.FillRect(0, 0, overlayW, overlayH, D2D1::ColorF(0, 0, 0, 0.55f));
	...
	renderer.FillRoundedRect(x, y, w, h, 12, Theme::BgSurface());
	renderer.DrawRoundedRect(x, y, w, h, 12, Theme::Error(), 1.4f);

	D2D1_COLOR_F glow = Theme::Error();
	glow.a = 0.18f;
	renderer.FillCircle(x + 54, y + 54, 28, glow);
	renderer.DrawText(L"\xE7BA", x + 42, y + 40, 24, 28, Theme::Error(), renderer.pFontIcon, Renderer::AlignCenter);

	renderer.DrawText(L"Missing Visual C++ runtime libraries", x + 92, y + 24, w - 116, 28, Theme::TextPrimary(), renderer.pFontBody);
	renderer.DrawText(L"AetherService needs the Visual C++ 2015-2022 Redistributable (x64) to run. Without it the driver will crash on launch. Click Download to grab the all-in-one pack from TechPowerUp and install it.",
	x + 92, y + 56, w - 116, 70, Theme::TextMuted(), renderer.pFontSmall);

	renderer.FillRoundedRect(x + 18, y + 140, w - 36, 56, 8, Theme::BgElevated());
	renderer.DrawRoundedRect(x + 18, y + 140, w - 36, 56, 8, Theme::BorderSubtle());
	renderer.DrawText(L"Missing DLLs", x + 34, y + 150, 140, 18, Theme::TextMuted(), renderer.pFontSmall);
	renderer.DrawText(missingRuntimeNames.empty() ? L"(unknown)" : missingRuntimeNames.c_str(), x + 34, y + 170, w - 68, 18, Theme::Error(), renderer.pFontSmall, Renderer::AlignLeft);

	float btnY = y + h - 50;
	runtimeMissingLaterBtn.Layout(x + w - 250, btnY, 100, 30, L"Ignore", false);
	if (runtimeMissingLaterBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		runtimeMissingModalOpen = false;
	}
	runtimeMissingLaterBtn.Draw(renderer);

	runtimeMissingOpenBtn.Layout(x + w - 140, btnY, 122, 30, L"Download", true);
	if (runtimeMissingOpenBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		OpenExternalUrl(L"https://www.techpowerup.com/download/visual-c-redistributable-runtime-package-all-in-one/");
	}
	runtimeMissingOpenBtn.Draw(renderer);
}
...
void AetherApp::DrawPluginManagerModal() {
	...
	renderer.FillRoundedRect(x + 18, y + 46, 142, 30, 6, Theme::AccentPrimary());
	renderer.DrawText(L"Aether Filters", x + 18, y + 46, 142, 30, D2D1::ColorF(0xFFFFFF), renderer.pFontSmall, Renderer::AlignCenter);
	pluginManagerTab = 0;

	std::wstring sourceFolder = L"Plugins";
	std::wstring sourceOwnerDisplay = GetSourceCatalogOwner(pluginRepoOwner);
	std::wstring sourceNameDisplay = GetSourceCatalogName(pluginRepoOwner, pluginRepoName);
	std::wstring sourceRefDisplay = GetSourceCatalogRef(pluginRepoOwner, pluginRepoName, pluginRepoRef);
	std::wstring source = sourceOwnerDisplay + L"/" + sourceNameDisplay + L" / " + sourceFolder + L" @ " + sourceRefDisplay;
	renderer.DrawText(source.c_str(), x + 174, y + 49, w - 360, 24, Theme::TextMuted(), renderer.pFontSmall);
	...
	if (pluginManagerSourceBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		...
		std::wstring visibleName = GetSourceCatalogName(pluginRepoOwner, pluginRepoName);
		std::wstring visibleRef = GetSourceCatalogRef(pluginRepoOwner, pluginRepoName, pluginRepoRef);
		wcscpy_s(pluginRepoOwnerInput.buffer, visibleOwner.c_str());
		pluginRepoOwnerInput.cursor = (int)visibleOwner.length();
		wcscpy_s(pluginRepoNameInput.buffer, visibleName.c_str());
		pluginRepoNameInput.cursor = (int)visibleName.length();
		wcscpy_s(pluginRepoRefInput.buffer, visibleRef.c_str());
		pluginRepoRefInput.cursor = (int)visibleRef.length();
	...
	renderer.FillRoundedRect(listX, listY, listW, listH, 6, Theme::BgDeep());
	renderer.DrawRoundedRect(listX, listY, listW, listH, 6, Theme::BorderSubtle());
	renderer.DrawText(L"Aether Filters", listX + 10, listY + 8, listW - 20, 20, Theme::TextSecondary(), renderer.pFontSmall);
	...
	if (!pluginCatalogEntries.empty() && selectedCatalogIndex >= 0 && selectedCatalogIndex < (int)pluginCatalogEntries.size()) {
		...
		const wchar_t* state = entry.installed
			? (entry.needsUpdate ? L"Installed, but repository source differs. Reinstall will rebuild and replace it." : L"Installed")
			: (entry.sourcePort ? L"Ready to download, build, and install" : L"Local/manual entry. Use Install DLL or Build Source.");
...
void AetherApp::DrawFilterPanel() {
	...
	if (filters.smoothingEnabled.value) {
		...
		filters.smoothingLatency.y = y; filters.smoothingLatency.x = cx; filters.smoothingLatency.width = hw;
		if (filters.smoothingLatency.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime)) {
			
			float iv = filters.smoothingLatency.value * 0.5f;
			if (iv < 1.0f) iv = 1.0f;
			if (iv > 16.0f) iv = 16.0f;
			filters.smoothingInterval.value = iv;
			filterChanged = true;
		}
		filters.smoothingLatency.Draw(renderer);
		y += 48;
	}

	
	filters.antichatterEnabled.y = y; filters.antichatterEnabled.x = cx;
	filterChanged |= filters.antichatterEnabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
	filters.antichatterEnabled.Draw(renderer); y += 30;
	if (filters.antichatterEnabled.value) {
		filters.antichatterStrength.y = y; filters.antichatterStrength.x = cx; filters.antichatterStrength.width = hw;
		...
		filters.antichatterMultiplier.y = y; filters.antichatterMultiplier.x = filterRightX; filters.antichatterMultiplier.width = hw;
	...
	filters.reconstructorEnabled.y = y; filters.reconstructorEnabled.x = cx;
	{
		...
		if (reconToggled && filters.reconstructorEnabled.value && filters.temporalEnabled.value)
			filters.temporalEnabled.value = false;
	}
	...
	sec.Layout(cx, y, cw, L"TEMPORAL RESAMPLER"); y += sec.Draw(renderer);
	filters.temporalEnabled.y = y; filters.temporalEnabled.x = cx;
	{
		bool temporalToggled = filters.temporalEnabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
		filterChanged |= temporalToggled;
		if (temporalToggled && filters.temporalEnabled.value && filters.reconstructorEnabled.value)
			filters.reconstructorEnabled.value = false;
	}
	filters.temporalEnabled.Draw(renderer); y += 30;
	if (filters.temporalEnabled.value) {
		filters.temporalPrediction.y = y; filters.temporalPrediction.x = cx;
		filterChanged |= filters.temporalPrediction.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.temporalPrediction.Draw(renderer);
		if (filterSingleColumn) y += 48;
		filters.temporalSmoothing.y = y; filters.temporalSmoothing.x = filterRightX;
		filterChanged |= filters.temporalSmoothing.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.temporalSmoothing.Draw(renderer);
		y += 48;
		filters.temporalReverseEma.y = y; filters.temporalReverseEma.x = cx;
		filterChanged |= filters.temporalReverseEma.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.temporalReverseEma.Draw(renderer);
		if (filterSingleColumn) y += 48;
	...
	if (false) {
	sec.Layout(cx, y, cw, L"PRESSURE CURVE"); y += sec.Draw(renderer);
	filters.pressureCurveEnabled.y = y; filters.pressureCurveEnabled.x = cx;
	filterChanged |= filters.pressureCurveEnabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
	filters.pressureCurveEnabled.Draw(renderer); y += 30;
	if (filters.pressureCurveEnabled.value) {
		filters.pressureExponent.y = y; filters.pressureExponent.x = cx;
		filterChanged |= filters.pressureExponent.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.pressureExponent.Draw(renderer);
		if (filterSingleColumn) y += 48;
		filters.pressureMin.y = y; filters.pressureMin.x = filterRightX;
		filterChanged |= filters.pressureMin.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.pressureMin.Draw(renderer);
		y += 48;
		filters.pressureMax.y = y; filters.pressureMax.x = cx;
		filterChanged |= filters.pressureMax.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime); filters.pressureMax.Draw(renderer);
		y += 48;
	}
	}
	...
	filterChanged |= aether.enabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
	aether.enabled.Draw(renderer); y += 30;
	if (aether.enabled.value) {
		aether.stabilizerEnabled.y = y; aether.stabilizerEnabled.x = cx;
		filterChanged |= aether.stabilizerEnabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
		aether.stabilizerEnabled.Draw(renderer); y += 30;
		if (aether.stabilizerEnabled.value) {
			aether.stabilizerStability.y = y; aether.stabilizerStability.x = cx;
			...
			if (filterSingleColumn) y += 48;
			aether.stabilizerSensitivity.y = y; aether.stabilizerSensitivity.x = filterRightX;
		...
		filterChanged |= aether.snappingEnabled.Update(mouseX, mouseY, mouseClicked, deltaTime);
		aether.snappingEnabled.Draw(renderer); y += 30;
		if (aether.snappingEnabled.value) {
			aether.snappingInner.y = y; aether.snappingInner.x = cx;
			...
			if (filterSingleColumn) y += 48;
			aether.snappingOuter.y = y; aether.snappingOuter.x = filterRightX;
	...
	if (rateMode.selected == 1) {
		...
		renderer.DrawText(
			L"Rate Limit only lowers the report rate. For a slow tablet that needs a higher rate, switch to Interpolation.",
...
int AetherApp::ConsoleHitTest(float x, float y) {
	if (consoleLayoutCount <= 0 || x < consoleRectX || x > consoleRectX + consoleRectW) return -1;
	if (y < consoleRectY || y > consoleRectY + consoleRectH) return -1;
	auto lines = driver.GetLogLines();
	if ((int)lines.size() != consoleLayoutCount || consoleLineH.size() != lines.size()) return -1;
	float totalH = 0;
	for (float hh : consoleLineH) totalH += hh;
	float viewportH = consoleRectH - 16;
	if (viewportH < 1) viewportH = 1;
	float maxScroll = totalH - viewportH;
	if (maxScroll < 0) maxScroll = 0;
	float off = consoleScrollY;
	if (off > maxScroll) off = maxScroll;
	if (off < 0) off = 0;
	float contentY = (y - (consoleRectY + 8)) + (totalH - off - viewportH);
	float acc = 0;
	for (int i = 0; i < (int)lines.size(); i++) {
		if (contentY < acc + consoleLineH[i]) return i;
		acc += consoleLineH[i];
	}
	return (int)lines.size() - 1;
}

void AetherApp::CopyConsoleSelectionToClipboard() {
	if (consoleSelStart < 0 || consoleSelEnd < 0 || !hWnd) return;
	auto lines = driver.GetLogLines();
	int a2 = consoleSelStart < consoleSelEnd ? consoleSelStart : consoleSelEnd;
	int b = consoleSelStart < consoleSelEnd ? consoleSelEnd : consoleSelStart;
	if (a2 < 0 || b >= (int)lines.size()) return;
	std::wstring text;
	for (int i = a2; i <= b; i++) {
	...
	if (g) {
		...
		if (p) {
			wmemcpy(p, text.c_str(), text.size() + 1);
			GlobalUnlock(g);
...
void AetherApp::DrawConsolePanel() {
	...
	// wrapped-height layout: recompute only when width or line count changes
	auto lines = driver.GetLogLines();
	float lineH = 16;
	float textW = cw - 24;
	if ((int)lines.size() != consoleLayoutCount || textW != consoleLayoutW) {
		consoleLineH.assign(lines.size(), lineH);
		for (size_t li = 0; li < lines.size(); li++) {
			std::wstring wl(lines[li].begin(), lines[li].end());
			float mw = 0, mh = 0;
			if (renderer.MeasureText(wl.c_str(), renderer.pFontMono, textW, &mw, &mh) && mh > lineH)
				consoleLineH[li] = mh + 2; // small gap so wrapped blocks do not touch
		}
		consoleLayoutCount = (int)lines.size();
		consoleLayoutW = textW;
	}
	consoleRectX = cx; consoleRectY = y; consoleRectW = cw; consoleRectH = consoleH;
	float totalH = 0;
	for (float hh : consoleLineH) totalH += hh;
	float viewportH = consoleH - 16;
	if (viewportH < 1) viewportH = 1;
	float maxScroll = totalH - viewportH;
	if (maxScroll < 0) maxScroll = 0;
	if (consoleScrollY > maxScroll) consoleScrollY = maxScroll;
	if (consoleScrollY < 0) consoleScrollY = 0;
	float yTopContent = totalH - consoleScrollY - viewportH;
	float prefix = 0;
	int startIdx = 0;
	for (int i = 0; i < (int)lines.size(); i++) {
		if (prefix + consoleLineH[i] > yTopContent) { startIdx = i; break; }
		prefix += consoleLineH[i];
	}
	float textY = y + 8 + (prefix - yTopContent);
	int selA = consoleSelStart < consoleSelEnd ? consoleSelStart : consoleSelEnd;
	int selB = consoleSelStart < consoleSelEnd ? consoleSelEnd : consoleSelStart;
	renderer.pRT->PushAxisAlignedClip(
		D2D1::RectF(cx, y, cx + cw, y + consoleH), D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
	for (int i = startIdx; i < (int)lines.size(); i++) {
		if (textY > y + consoleH - 8) break;
		if (selA >= 0 && i >= selA && i <= selB && i < (int)consoleLineH.size()) {
			D2D1_COLOR_F ac = Theme::AccentPrimary();
			renderer.FillRect(cx + 8, textY, cw - 16, consoleLineH[i],
				D2D1::ColorF(ac.r, ac.g, ac.b, 0.20f));
		}
		...
		renderer.DrawText(wline.c_str(), cx + 12, textY, textW, consoleLineH[i], color, renderer.pFontMono);
		textY += consoleLineH[i];
	}
		renderer.pRT->PopAxisAlignedClip();
...
void AetherApp::DoctorScanConflicts() {
	doctorConflicts.clear();
	static const wchar_t* names[] = {
		L"WTabletServicePro.exe", L"WTabletServiceCon.exe", L"TabletInputService.exe",
		L"WacomHost.exe", L"PenTablet.exe", L"HuionTablet.exe",
		L"TabletDriver.exe", L"TabletDriverGUI.exe", L"GaomonTablet.exe"
	};
	HANDLE snap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	if (snap == INVALID_HANDLE_VALUE) { doctorScanned = true; return; }
	PROCESSENTRY32W pe = {};
	pe.dwSize = sizeof(pe);
		...
		do {
			for (size_t i = 0; i < sizeof(names)/sizeof(names[0]); i++) {
				if (_wcsicmp(pe.szExeFile, names[i]) == 0) {
					bool dup = false;
					for (auto& d : doctorConflicts) if (_wcsicmp(d.c_str(), names[i]) == 0) { dup = true; break; }
					if (!dup) doctorConflicts.push_back(names[i]);
				}
			}
...
void AetherApp::DoctorKillProcess(const std::wstring& name) {
	wchar_t args[256];
	swprintf_s(args, L"/C taskkill /F /IM \"%s\"", name.c_str());
	wchar_t buf[300];
	wcscpy_s(buf, args);
	STARTUPINFOW si = {};
	si.cb = sizeof(si);
	PROCESS_INFORMATION pi = {};
	bool ok = CreateProcessW(L"C:\\Windows\\System32\\cmd.exe", buf, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi) != 0;
	if (ok) {
...
void AetherApp::DrawDoctorPanel() {
	...
	float cw = Theme::Runtime::WindowWidth - Theme::Size::SidebarWidth - Theme::Size::Padding * 2;
	float yStart = Theme::Size::HeaderHeight + Theme::Size::Padding;
	float y = yStart - doctorScrollY;
	SectionHeader sec;

	float livePressure = driver.penActive.load() ? Clamp(driver.penPressure.load(), 0.0f, 1.0f) : 0.0f;
	doctorPressureHistory.push_back(livePressure);
	if ((int)doctorPressureHistory.size() > 420)
		doctorPressureHistory.erase(doctorPressureHistory.begin(), doctorPressureHistory.begin() + ((int)doctorPressureHistory.size() - 420));
	if (livePressure > doctorPeakPressure) doctorPeakPressure = livePressure;

	sec.Layout(cx, y, cw, L"DRIVER"); y += sec.Draw(renderer);
	wchar_t buf[256];

	auto statusRow = [&](const wchar_t* label, const wchar_t* value, bool good) {
		renderer.DrawText(label, cx + 4, y, 170, 20, Theme::TextMuted(), renderer.pFontSmall);
		D2D1_COLOR_F col = good ? Theme::Success() : Theme::Warning();
		renderer.FillCircle(cx + 188, y + 10, 3.0f, col);
		D2D1_COLOR_F vcol = good ? Theme::TextSecondary() : Theme::Warning();
		renderer.DrawText(value, cx + 200, y, cw - 210, 20, vcol, renderer.pFontSmall);
		y += 24;
	};

	bool svc = driver.isConnected.load();
	bool tabOk = svc && driver.tabletConnected.load();
	statusRow(L"Service", svc ? L"Running" : L"Not running", svc);
	if (driver.tabletName.empty()) {
		statusRow(L"Tablet", L"-", false);
	} else {
		std::wstring name(driver.tabletName.begin(), driver.tabletName.end());
		swprintf_s(buf, L"%s", name.c_str());
		statusRow(L"Tablet", tabOk ? buf : L"Detected, not connected", tabOk);
	}
	swprintf_s(buf, L"%.0f Hz", driver.penHz.load());
	statusRow(L"Report rate", buf, driver.penHz.load() > 50.0f);
	int latN = driver.latencySamples.load();
	if (latN > 0) {
		swprintf_s(buf, L"Average %.2f ms | Usually below %.2f ms | Worst spike %.2f ms (%d samples)",
			driver.latencyAvgMs.load(), driver.latencyP99Ms.load(), driver.latencyMaxMs.load(), latN);
		statusRow(L"Latency", buf, driver.latencyAvgMs.load() < 8.0f);
	} else {
		statusRow(L"Latency", L"No samples yet", false);
	}
	bool vmultiNeeded = (outputMode.selected == 2 || outputMode.selected == 3);
	statusRow(L"VMulti", vmultiInstalled ? L"Installed" : L"Not installed", !vmultiNeeded || vmultiInstalled);
	if (!vmultiInstalled && vmultiNeeded) {
		renderer.DrawText(L"Only required for Windows Ink and Raw Absolute modes.",
			cx + 4, y, cw - 8, 18, Theme::TextMuted(), renderer.pFontSmall);
		y += 22;
	}
	y += 12;

	// --- input health verdict: turns the raw numbers above into a verdict + fixes ---
	sec.Layout(cx, y, cw, L"INPUT HEALTH"); y += sec.Draw(renderer);
	{
		bool rateOk = driver.penHz.load() >= 100.0f;
		bool latOk = latN > 0 && driver.latencyAvgMs.load() < 8.0f;
		bool spikes = latN > 0 && (driver.latencyP99Ms.load() >= 20.0f || driver.latencyMaxMs.load() >= 50.0f);
		bool vmultiOk = !vmultiNeeded || vmultiInstalled;
		bool conflictsFound = doctorScanned && !doctorConflicts.empty();

		int score = (svc ? 1 : 0) + (tabOk ? 1 : 0) + (rateOk ? 1 : 0)
			+ (latOk ? 1 : 0) + (!spikes ? 1 : 0) + (vmultiOk ? 1 : 0) + (!conflictsFound ? 1 : 0);
		const wchar_t* verdict;
		D2D1_COLOR_F vc;
		if (score >= 6)      { verdict = L"GOOD";   vc = Theme::Success(); }
		else if (score >= 4) { verdict = L"WATCH";  vc = Theme::Warning(); }
		else                 { verdict = L"POOR";   vc = Theme::Error(); }
		// verdict pill, same style as the Conflicting Drivers result row
		renderer.FillRoundedRect(cx + 4, y, cw - 8, 26, 5, D2D1::ColorF(vc.r, vc.g, vc.b, 0.12f));
		if (latN > 0)
			swprintf_s(buf, L"%s  -  average %.2f ms  |  spikes under %.2f ms  |  %.0f Hz  |  %d of 7 checks passed", verdict, driver.latencyAvgMs.load(), driver.latencyP99Ms.load(), driver.penHz.load(), score);
		else
			swprintf_s(buf, L"%s  -  %.0f Hz  |  %d of 7 checks passed (draw on the tablet to measure latency)", verdict, driver.penHz.load(), score);
		renderer.DrawText(buf, cx + 16, y + 4, cw - 28, 18, vc, renderer.pFontSmall);
		y += 30;

		auto advice = [&](const wchar_t* text) {
			renderer.FillCircle(cx + 10, y + 8, 3.0f, Theme::Warning());
			renderer.DrawText(text, cx + 22, y - 1, cw - 30, 20, Theme::TextSecondary(), renderer.pFontSmall);
			y += 24;
		};
		if (!svc) advice(L"Service is not running - press Start Driver.");
		else if (!tabOk) advice(L"Tablet is not connected - open the Area tab and press Connect.");
		if (tabOk && !rateOk) advice(L"Report rate is low - try a rear motherboard USB port and disable USB selective suspend in the power plan.");
		if (latN > 0 && !latOk) advice(L"The filter pipeline adds delay on average - lower the Smoothing / Aether Glide latency values or disable unused timed filters.");
		if (spikes) advice(L"Latency spikes detected - enable Overclock (0.5 ms timer), close background CPU hogs; on laptops switch the power plan to High Performance.");
		if (!vmultiOk) advice(L"VMulti is missing - install it for Windows Ink / Raw Absolute output modes.");
		if (conflictsFound) advice(L"Another tablet driver is running - close or uninstall it (see Conflicting Drivers below).");
		if (score >= 6 && latN > 0) {
			renderer.FillRoundedRect(cx + 4, y, cw - 8, 24, 5, D2D1::ColorF(0.2f, 0.6f, 0.3f, 0.12f));
			renderer.DrawText(L"Pipeline looks healthy - nothing to fix.", cx + 16, y + 3, cw - 24, 18, Theme::Success(), renderer.pFontSmall);
			y += 28;
		}
	}

	sec.Layout(cx, y, cw, L"CONFLICTING DRIVERS"); y += sec.Draw(renderer);
	renderer.DrawText(L"Other tablet drivers (Wacom, Huion, Gaomon, XP-Pen) can grab the device or inject extra cursor input. Kill them if Aether misbehaves.",
		cx + 4, y, cw - 8, 36, Theme::TextMuted(), renderer.pFontSmall);
	y += 40;
	doctorScanBtn.Layout(cx + 4, y, 110, 26, L"Scan", false);
	if (doctorScanBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) DoctorScanConflicts();
	doctorScanBtn.Draw(renderer);
	y += 34;
	if (doctorScanned) {
		if (doctorConflicts.empty()) {
			renderer.FillRoundedRect(cx + 4, y, cw - 8, 24, 5, D2D1::ColorF(0.2f, 0.6f, 0.3f, 0.12f));
			renderer.DrawText(L"No conflicting tablet drivers found.", cx + 16, y, cw - 24, 24, Theme::Success(), renderer.pFontSmall);
			y += 28;
		} else {
			for (size_t i = 0; i < doctorConflicts.size(); i++) {
				const std::wstring& n = doctorConflicts[i];
				renderer.FillRoundedRect(cx + 4, y, cw - 8, 26, 5, Theme::BgElevated());
				renderer.DrawRoundedRect(cx + 4, y, cw - 8, 26, 5, Theme::BorderSubtle());
				renderer.DrawText(n.c_str(), cx + 16, y, cw - 120, 26, Theme::Warning(), renderer.pFontSmall);
				float kbX = cx + cw - 92;
				float kbY = y + 3;
				bool kbHover = PointInRect(mouseX, mouseY, kbX, kbY, 84, 20);
				D2D1_COLOR_F kbBg = kbHover ? Theme::Error() : Theme::BgElevated();
				renderer.FillRoundedRect(kbX, kbY, 84, 20, 4, kbBg);
				renderer.DrawRoundedRect(kbX, kbY, 84, 20, 4, Theme::BorderSubtle());
				renderer.DrawText(L"Kill", kbX, kbY, 84, 20, Theme::TextSecondary(), renderer.pFontSmall, Renderer::AlignCenter);
				if (kbHover && mouseClicked) DoctorKillProcess(n);
				y += 30;
			}
		}
	}
	...
	// live pressure graph: the history is collected at the top of this function
	sec.Layout(cx, y, cw, L"LIVE PRESSURE"); y += sec.Draw(renderer);
	{
		float graphH = 90.0f;
		renderer.FillRoundedRect(cx + 4, y, cw - 8, graphH, 5, Theme::BgElevated());
		renderer.DrawRoundedRect(cx + 4, y, cw - 8, graphH, 5, Theme::BorderSubtle());
		int n = (int)doctorPressureHistory.size();
		if (n > 1) {
			float gw = cw - 16.0f, gx = cx + 8.0f;
			for (int i = 1; i < n; i++) {
				float x0 = gx + gw * (float)(i - 1) / 420.0f;
				float x1 = gx + gw * (float)i / 420.0f;
				float y0 = y + graphH - 4.0f - doctorPressureHistory[i - 1] * (graphH - 8.0f);
				float y1 = y + graphH - 4.0f - doctorPressureHistory[i] * (graphH - 8.0f);
				renderer.DrawLine(x0, y0, x1, y1, Theme::AccentPrimary(), 1.5f);
			}
		}
		...
		if (doctorResetBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
			...
			doctorPressureHistory.clear();
		}
		doctorResetBtn.Draw(renderer);
		y += graphH + 40;
	}

	sec.Layout(cx, y, cw, L"JITTER TEST"); y += sec.Draw(renderer);
	renderer.DrawText(L"Rest the pen on the tablet and hold it as still as you can. Five seconds, keep the tip down.",
		cx, y, cw, 32, Theme::TextMuted(), renderer.pFontSmall);
	y += 36;

	if (doctorJitterRunning) {
		doctorJitterElapsed += deltaTime;
		float px = driver.penX.load();
		float py = driver.penY.load();
		float mmX = driver.maxX > 0 ? (driver.tabletWidth / (float)driver.maxX) : 0.0f;
		float mmY = driver.maxY > 0 ? (driver.tabletHeight / (float)driver.maxY) : 0.0f;

		doctorJitterX.push_back(px);
		doctorJitterY.push_back(py);

		float dx = (px - doctorJitterX[0]) * mmX;
		float dy = (py - doctorJitterY[0]) * mmY;
		if (sqrtf(dx * dx + dy * dy) > 3.0f) doctorJitterMoved = true;

		if (doctorJitterElapsed >= 5.0f || doctorJitterX.size() > 3000) {
			int n = (int)doctorJitterX.size();
			double sx = 0, sy = 0;
			for (int i = 0; i < n; i++) { sx += doctorJitterX[i]; sy += doctorJitterY[i]; }
			float mx = (float)(sx / n), my = (float)(sy / n);
			float sum = 0, worst = 0, lastD = 0;
			int jumps = 0;
			for (int i = 0; i < n; i++) {
				float ddx = (doctorJitterX[i] - mx) * mmX;
				float ddy = (doctorJitterY[i] - my) * mmY;
				float dist = sqrtf(ddx * ddx + ddy * ddy);
				sum += dist;
				if (dist > worst) worst = dist;
				if (i > 0 && fabsf(dist - lastD) > 0.35f) jumps++;
				lastD = dist;
			}
			doctorJitterAvg = sum / n;
			doctorJitterMax = worst;
			doctorJitterCount = n;
			doctorJitterJumpFrac = n > 1 ? (float)jumps / (n - 1) : 0.0f;
			doctorJitterRunning = false;
		}
	}

	doctorJitterBtn.Layout(cx + 4, y, 150, 26, doctorJitterRunning ? L"Stop" : L"Start test", false);
	if (doctorJitterBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
		if (doctorJitterRunning) {
			doctorJitterRunning = false;
		} else {
			doctorJitterRunning = true;
			doctorJitterElapsed = 0;
			doctorJitterX.clear();
			doctorJitterY.clear();
			doctorJitterMoved = false;
			doctorJitterJumpFrac = 0;
			doctorJitterAvg = doctorJitterMax = -1;
			doctorJitterCount = 0;
		}
	}
	doctorJitterBtn.Draw(renderer);

	if (doctorJitterRunning) {
		swprintf_s(buf, L"Recording... %.1fs", doctorJitterElapsed);
		renderer.DrawText(buf, cx + 170, y + 4, cw - 180, 20, Theme::AccentPrimary(), renderer.pFontSmall);
	} else if (doctorJitterAvg >= 0) {
		if (doctorJitterMoved) {
			renderer.DrawText(L"Pen moved too much - hold it still and try again",
				cx + 170, y + 4, cw - 180, 20, Theme::Warning(), renderer.pFontSmall);
		} else {
			bool good = doctorJitterAvg < 0.15f && doctorJitterJumpFrac < 0.01f;
			swprintf_s(buf, L"Drift %.2f mm avg, %.2f mm worst - jumps %.1f%% of %d - %s",
				doctorJitterAvg, doctorJitterMax, doctorJitterJumpFrac * 100.0f, doctorJitterCount,
				good ? L"clean" : L"noisy");
			renderer.DrawText(buf, cx + 170, y + 4, cw - 180, 20, good ? Theme::Success() : Theme::Warning(), renderer.pFontSmall);
		}
	}
	y += 40;
...
void AetherApp::DrawBrushPanel() {
	...
	float ddRectY = 0.0f, ddRectH = 0.0f;
	bool ddVisible = false;
	if (openActionDropdown >= 0 && openActionDropdown < kActionHotkeyCount) {
		ddRectY = y + openActionDropdown * (rowH + rowGap) + rowH + 2.0f;
		ddRectH = kActionHotkeyNameCount * 26.0f;
		ddVisible = true;
		if (mouseClicked && PointInRect(mouseX, mouseY, cx, ddRectY, actionW, ddRectH)) {
			ActionHotkey& h = actionHotkeys[openActionDropdown];
			int picked = -1;
			for (int j = 0; j < kActionHotkeyNameCount; ++j) {
				float itemY = ddRectY + j * 26.0f;
				if (PointInRect(mouseX, mouseY, cx, itemY, actionW, 26.0f)) { picked = j; break; }
			}
			if (picked >= 0) {
				h.actionIndex = picked;
				openActionDropdown = -1;
				SendActionHotkeysToDriver();
				AutoSaveConfig();
			}
			mouseClicked = false;
		} else if (mouseClicked && !PointInRect(mouseX, mouseY, cx, ddRectY, actionW, ddRectH)) {
			openActionDropdown = -1;
			mouseClicked = false;
		}
	}
	...
	if (ddVisible && openActionDropdown >= 0) {
		float ddBottom = (ddRectY + ddRectH + 8.0f) - yStart + brushScrollY;
		if (ddBottom > contentEnd) contentEnd = ddBottom;
	}
	brushContentH = contentEnd + 64;
	if (changed) {
		ApplyAllSettings();
	}
	ClampScrollOffsets();
...
void AetherApp::DrawAboutPanel() {
	...
	sec.Layout(cx, y, cw, L"ABOUT"); y += sec.Draw(renderer);

	// floating logo: gentle bob + breathing scale
	{
		float size = 76.0f;
		float bob = sinf(aboutAnimT * 1.3f) * 5.0f;
		float pulse = 1.0f + 0.045f * sinf(aboutAnimT * 0.8f);
		float drawSize = size * pulse;
		float lx = cx + (cw - drawSize) * 0.5f;
		float ly = y + 14.0f + bob;
		...
		if (renderer.pLogoBitmap) {
			...
			tint.b = tint.b * 0.6f + Theme::Base::TextPriB * 0.4f;
			tint.a = 1.0f;
			renderer.DrawBitmapTinted(renderer.pLogoBitmap, lx, ly, drawSize, drawSize, tint, 1.0f);
		}
		else {
			D2D1_COLOR_F glyphCol = Theme::AccentPrimary();
			glyphCol.r = glyphCol.r * 0.6f + Theme::Base::TextPriR * 0.4f;
			glyphCol.g = glyphCol.g * 0.6f + Theme::Base::TextPriG * 0.4f;
			glyphCol.b = glyphCol.b * 0.6f + Theme::Base::TextPriB * 0.4f;
			renderer.DrawText(L"\x2726", lx, ly, drawSize, drawSize, glyphCol, renderer.pFontTitle, Renderer::AlignCenter);
		}
		y += size + 52.0f;
	}

	wchar_t buf[200];
	renderer.DrawText(L"AetherGUI", cx + 4, y, cw - 8, 26, Theme::TextPrimary(), renderer.pFontHeading, Renderer::AlignCenter);
	y += 34;
	renderer.DrawText(L"Native C++ tablet driver for Windows. No .NET, no Electron - Win32, Direct2D and low latency all the way down.",
		cx + 4, y, cw - 8, 20, Theme::TextSecondary(), renderer.pFontSmall, Renderer::AlignCenter);
	y += 32;

	// update check: same GitHub lookup as the startup check, centered row
	{
		float btnW = 170.0f;
		float btnX = cx + (cw - btnW) * 0.5f;
		aboutUpdateBtn.Layout(btnX, y, btnW, 28, L"Check for updates", false);
		if (aboutUpdateBtn.Update(mouseX, mouseY, mouseClicked, deltaTime)) {
			aboutUpdateState.store(1);
			HWND hwnd = hWnd;
				...
				if (FetchLatestRelease(latest)) {
					if (IsVersionNewer(latest.tag, AETHERGUI_VERSION)) {
						...
						info->releaseUrl = latest.htmlUrl.empty()
							? std::wstring(AETHERGUI_GITHUB_RELEASES_URL)
							: Utf8ToWide(latest.htmlUrl);
						aboutUpdateState.store(3);
						PostMessageW(hwnd, WM_AETHER_UPDATE_AVAILABLE, 0, reinterpret_cast<LPARAM>(info));
						return;
					}
				}
				aboutUpdateState.store(2);
			}).detach();
		}
		aboutUpdateBtn.Draw(renderer);
		y += 32;

		const wchar_t* updState = L"";
		switch (aboutUpdateState.load()) {
		case 1: updState = L"Checking..."; break;
		case 2: updState = L"You are up to date"; break;
		case 3: updState = L"Update available"; break;
		}
		if (updState[0])
			renderer.DrawText(updState, cx + 4, y, cw - 8, 18, Theme::TextMuted(), renderer.pFontSmall, Renderer::AlignCenter);
		y += 26;
	}
	...
	swprintf_s(buf, L"Service: %s   |   VMulti: %s",
		driver.isConnected.load() ? L"running" : L"not running",
		vmultiInstalled ? L"installed" : L"not installed");
	centerLine(buf, Theme::TextSecondary());
...
void AetherApp::DrawStatusBar() {
	...
	D2D1_COLOR_F statusColor = (driver.isConnected && driver.tabletConnected.load()) ? Theme::Success() : Theme::TextMuted();
	renderer.FillCircle(18, y + h * 0.5f, 3.0f, statusColor);
	bool tabletOk2 = driver.isConnected && driver.tabletConnected.load();
	const wchar_t* statusText = tabletOk2 ? L"Connected" : (driver.isConnected ? L"No Tablet" : L"Not connected");
	...
	if (w > 720.0f) {
		...
		renderer.DrawText(L"Interpolation", w - 208, y, 104, h, Theme::TextMuted(), renderer.pFontSmall, Renderer::AlignRight);
...
void AetherApp::GetThemeSlotColor(Theme::ThemeData& t, int slot, float& r, float& g, float& b) {
	...
	case 6: r = t.success[0]; g = t.success[1]; b = t.success[2]; break;
	}
}
...
void AetherApp::SetThemeSlotColor(Theme::ThemeData& t, int slot, float r, float g, float b) {
	...
	case 6: t.success[0]=r; t.success[1]=g; t.success[2]=b; break;
	}
}
...
void AetherApp::DrawThemeSelector(float x, float& y, float w) {
	...
	if (editingTheme >= 0 && editingTheme < uiThemeCount && editingSlot >= 0) {
		...
		if (slotPicker.Update(mouseX, mouseY, mouseDown, mouseClicked, deltaTime)) {
			...
			if (editingTheme >= 0 && editingTheme < uiThemeCount)
				themeEdited[editingTheme] = true;
...
void AetherApp::DrawConfigManager(float x, float& y, float w) {
...

	{
		const wchar_t* hint =
			...
			? L"Press a key combination to bind  (Esc to cancel)"
			: L"Click a slot's badge to set hotkey";
		renderer.DrawText(hint, x + 4, y, w - 8, 22,
			capturingHotkeySlot >= 0 ? Theme::TextPrimary() : Theme::TextMuted(),
			renderer.pFontSmall);
		y += 26;
	}
	...
	for (int i = 0; i < (int)configEntries.size(); i++) {
		...
		if (i < kConfigHotkeyCount) {
			bool capturing = (capturingHotkeySlot == i);
			std::wstring label;
			if (capturing)             label = L"press keys...";
			else if (configHotkeys[i].vk == 0) label = L"unbinded";
			else                       label = HotkeyLabelForSlot(i + 1);

			D2D1_COLOR_F bg = capturing
				? Theme::AccentPrimary()
				: (hotkeyHovered ? Theme::AccentPrimary() : Theme::AccentDim());
			renderer.FillRoundedRect(hotkeyX, hotkeyY, hotkeyW, hotkeyH, 5, bg);
			renderer.DrawRoundedRect(hotkeyX, hotkeyY, hotkeyW, hotkeyH, 5,
				(capturing || hotkeyHovered) ? Theme::BorderAccent() : Theme::BorderSubtle());
			renderer.DrawText(label.c_str(), hotkeyX, hotkeyY, hotkeyW, hotkeyH,
				(capturing || hotkeyHovered) ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
				renderer.pFontSmall, Renderer::AlignCenter);
		}
		...
		renderer.DrawRoundedRect(saveBtnX, btnY, miniW, btnH, 5,
			saveHovered ? Theme::BorderAccent() : Theme::BorderSubtle());
		renderer.DrawText(L"Save", saveBtnX, btnY, miniW, btnH,
			saveHovered ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
			renderer.pFontSmall, Renderer::AlignCenter);

		D2D1_COLOR_F loadBg = loadHovered ? Theme::AccentPrimary() : Theme::AccentDim();
		renderer.FillRoundedRect(loadBtnX, btnY, miniW, btnH, 5, loadBg);
		renderer.DrawRoundedRect(loadBtnX, btnY, miniW, btnH, 5,
			loadHovered ? Theme::BorderAccent() : Theme::BorderSubtle());
		...
		renderer.FillRoundedRect(deleteBtnX, btnY, miniW, btnH, 5, deleteBg);
		renderer.DrawRoundedRect(deleteBtnX, btnY, miniW, btnH, 5,
			deleteHovered ? Theme::Error() : Theme::BorderSubtle());
		renderer.DrawText(L"Delete", deleteBtnX, btnY, miniW, btnH,
		...
		if (mouseClicked) {
			...
			if (saveHovered) {
				...
				if (res == IDYES) {
					std::wstring victim = configEntries[i].path;
					DeleteFileW(victim.c_str());
					if (_wcsicmp(victim.c_str(), activeConfigPath.c_str()) == 0) {
						activeConfigPath.clear();
						SaveLastLoadedMarker();
					}
					selectedConfigIndex = -1;
...

	{
		...
		float toggleX = x;
		float toggleY = y;
		bool toggleHover = PointInRect(mouseX, mouseY, toggleX, toggleY, toggleW, toggleH);
		std::wstring label = appProfilesEnabled ? L"Auto-switch: ON" : L"Auto-switch: OFF";
		D2D1_COLOR_F bg = appProfilesEnabled
			? (toggleHover ? Theme::AccentPrimary() : Theme::AccentDim())
			: (toggleHover ? Theme::BgHover() : Theme::BgElevated());
		renderer.FillRoundedRect(toggleX, toggleY, toggleW, toggleH, 5, bg);
		renderer.DrawRoundedRect(toggleX, toggleY, toggleW, toggleH, 5,
			(appProfilesEnabled || toggleHover) ? Theme::BorderAccent() : Theme::BorderSubtle());
		renderer.DrawText(label.c_str(), toggleX, toggleY, toggleW, toggleH,
			(appProfilesEnabled || toggleHover) ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
			renderer.pFontSmall, Renderer::AlignCenter);
		...
		D2D1_COLOR_F addBg = addHover ? Theme::AccentPrimary() : Theme::AccentDim();
		renderer.FillRoundedRect(addX, toggleY, addW, toggleH, 5, addBg);
		renderer.DrawRoundedRect(addX, toggleY, addW, toggleH, 5,
			addHover ? Theme::BorderAccent() : Theme::BorderSubtle());
		std::wstring addLabel;
		if (lastForeignForeground.empty())
			addLabel = L"+ Add  (focus another window first)";
		else
			addLabel = L"+ Add  " + lastForeignForeground;
		renderer.DrawText(addLabel.c_str(), addX, toggleY, addW, toggleH,
			addHover ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
			renderer.pFontSmall, Renderer::AlignCenter);
		if (addHover && mouseClicked) AddAppProfileFromForeground();
	...
	if (appProfiles.empty()) {
		renderer.FillRoundedRect(x, y, w, 28, 6, Theme::BgElevated());
		renderer.DrawRoundedRect(x, y, w, 28, 6, Theme::BorderSubtle());
		renderer.DrawText(L"No app profiles yet. Click '+ Add current foreground' to map one.",
			x + 10, y, w - 20, 28, Theme::TextMuted(), renderer.pFontSmall);
		y += 34.0f;
		return;
	}
	...
	float appRowH = 28.0f;
	int removeIdx = -1;
	for (int i = 0; i < (int)appProfiles.size(); ++i) {
		float rowY = y + i * (appRowH + 4.0f);
		renderer.FillRoundedRect(x, rowY, w, appRowH, 6, Theme::BgElevated());
		renderer.DrawRoundedRect(x, rowY, w, appRowH, 6, Theme::BorderSubtle());

		float removeW = 70.0f;
		float removeX = x + w - removeW - 8.0f;
		...
		std::wstring line = appProfiles[i].processName + L"  ->  " + cfgName;
		renderer.DrawText(line.c_str(), x + 10, rowY, removeX - x - 18, appRowH,
			Theme::TextPrimary(), renderer.pFontSmall);

		D2D1_COLOR_F rmBg = removeHover
			? Theme::Error()
			: D2D1::ColorF(Theme::Error().r, Theme::Error().g, Theme::Error().b, 0.18f);
		renderer.FillRoundedRect(removeX, btnY, removeW, btnH, 5, rmBg);
		renderer.DrawRoundedRect(removeX, btnY, removeW, btnH, 5,
			removeHover ? Theme::BorderAccent() : Theme::BorderSubtle());
		renderer.DrawText(L"Remove", removeX, btnY, removeW, btnH,
			removeHover ? D2D1::ColorF(0xFFFFFF) : Theme::TextPrimary(),
			renderer.pFontSmall, Renderer::AlignCenter);

		if (mouseClicked && removeHover) {
			removeIdx = i;
		}
	}
	if (removeIdx >= 0) {
		appProfiles.erase(appProfiles.begin() + removeIdx);
		SaveAppProfiles();
	}
	y += (float)appProfiles.size() * (appRowH + 4.0f) + 6.0f;
}
