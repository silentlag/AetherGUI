...
// set by the DebugReports command, read by the tablet thread
extern atomic<bool> g_debugReports;
...
bool ProcessCommand(CommandLine *cmd) {
	...
	if (cmd->is("Tablet")) {
		...
		if (cmd->valueCount == 3) {
		...
		else if (cmd->valueCount >= 4 && cmd->valueCount <= 10) {
			...
			int interfaceNumber = cmd->GetInt(9, -1);

			if (tablet == NULL) {

				tablet = new Tablet(vendorID, productID, usagePage, usage, inputReportLength, stringId, stringMatch, stringId2, stringMatch2, interfaceNumber);
		...
		else if (cmd->GetStringLower(0, "") == "hiddigitizer") {
			tablet->settings.type = TabletSettings::TypeHidDigitizer;
		}
		...
		if (enabled) {
			...
			// timer resolution: requested globally in GlobalInit()
			...
			// timer resolution: owned by GlobalInit(), not released here
	...
	else if (cmd->is("DebugReports")) {
		string v = cmd->GetStringLower(0, "");
		bool on = (v == "true" || v == "1" || v == "on");
		g_debugReports.store(on);
		LOG_INFO("Raw report dump: %s\n", on ? "on" : "off");
	}

	else if (cmd->is("Preset")) {
		string name = cmd->GetStringLower(0, "");
		if (name.empty()) {
			LOG_WARNING("Preset requires: <name>\n");
			return true;
		}
		printf("[STATUS] PRESET %s\n", name.c_str());
		LOG_INFO("Preset [%s] requested.\n", name.c_str());
	}
