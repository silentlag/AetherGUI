...
Tablet::Tablet(USHORT vendorId, USHORT productId, USHORT usagePage, USHORT usage, int inputReportLength, int stringId, string stringMatch, int stringId2, string stringMatch2, int interfaceNumber) : Tablet() {

	hidDevice = new HIDDevice(vendorId, productId, usagePage, usage, inputReportLength, stringId, stringMatch, stringId2, stringMatch2, interfaceNumber);
...
// plugins that need physical units (e.g. Radial Follow) receive the

// tablet-units -> mm scale; unknown keys are ignored by other plugins

void Tablet::PushPluginUnitScales() {

	if (settings.maxX < 2 || settings.maxY < 2 || settings.width <= 1.0 || settings.height <= 1.0) return;

	// state.position is ALREADY in mm (raw * invMaxX * width).
	// Pushing width/maxX compressed distances (e.g. 100x) - use 1:1.
	for (size_t i = 0; i < pluginFilters.size(); i++) {

		pluginFilters[i]->SetDoubleOption("tablet.mmScaleX", 1.0);

		pluginFilters[i]->SetDoubleOption("tablet.mmScaleY", 1.0);
...
int Tablet::ReadPosition() {
	...
	lastReportLen = settings.reportLength;

	if (lastReportLen > (int)sizeof(lastReport)) lastReportLen = (int)sizeof(lastReport);

	memcpy(lastReport, buffer, lastReportLen);
